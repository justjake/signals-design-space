...
function controlledThenable<T>() {
  ...
  const thenable = {
    then(onFulfilled: (value: T) => unknown, onRejected: (reason: unknown) => unknown): undefined {
      fulfill = onFulfilled
      reject = onRejected
      return undefined
    },
...
test("detached async records contain only execution state", () => {
  ...
  expect(Object.keys(box)).toEqual(["status", "result", "parkedNodes", "parkedSuspensions"])
})
