...
for (const { section, cases } of testSuite) {
	describe(`cosignals-arena :: ${section}`, () => {
		for (const [name, fn] of Object.entries(cases)) {
			...
			// an optimization; the Cosignals Arena correctness cases below replace these
			...
			// Cosignals Arena's effect is two functions (a pure tracked compute and an
			...
			// mid-body — that the shim cannot reproduce. The Cosignals Arena effect
...
describe('cosignals-arena :: Computed Evaluation', () => {
	/**
	 * No read occurs between the two writes. Cosignals Arena may conservatively
...
describe('cosignals-arena :: Batching / Transaction', () => {
	/**
...
describe('cosignals-arena :: Effect Lifecycle (split-effect contract)', () => {
	/**
