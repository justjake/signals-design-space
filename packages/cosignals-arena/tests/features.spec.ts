...
import * as cosignalsArena from "../src/index.ts"
import {
  createComputed,
  createEffect,
  isPending,
...
test("public execution controls are the graph functions", () => {
  expect(cosignalsArena.effectScope).toBe(makeScope)
  expect(cosignalsArena.batch).toBe(graphBatch)
  expect(cosignalsArena.startBatch).toBe(graphStartBatch)
  expect(cosignalsArena.endBatch).toBe(graphEndBatch)
  expect(cosignalsArena.untrack).toBe(graphUntracked)
  expect(cosignalsArena.flushScheduledEffects).toBe(graphFlushScheduledEffects)
})
...
describe("computed policy and APIs", () => {
  test("factories return graph nodes with one shared method prototype", () => {
    ...
    expect(cosignalsArena).not.toHaveProperty("Atom")
    expect(cosignalsArena).not.toHaveProperty("ReducerAtom")
    expect(cosignalsArena).not.toHaveProperty("Computed")
    expect(cosignalsArena).not.toHaveProperty("signal")
    expect(cosignalsArena).not.toHaveProperty("computed")
    expect(cosignalsArena.createAtom).toBe(createAtom)
    expect(cosignalsArena.createReducerAtom).toBe(createReducerAtom)
    expect(cosignalsArena.createComputed).toBe(createComputed)
  })
