...
export function appendDraftIntent(
  ...
  const tracer = activeTracer
  if (tracer !== null) {
    cause = tracer.emitEvent(kind, cell, tracer.getCause(draft), {
      draftId: draft.id,
    })
    tracer.setDraftWrite(draft, cause)
    tracer.setCause(cell, cause)
  }
  ...
  if (intent.kind === "update") {
    try {
      ...
      if (cutoffWorld === null && activeTracer !== null) {
        activeTracer.emitEvent("callback-error", cell, currentCause, { error, phase: "updater" })
      }
      throw error
    }
  }
