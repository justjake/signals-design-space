# signals-royale-fx2-dalien — REPORT

> Historical naming: `signals-royale-fx2-dalien` is now named `cosignals-arena`.

Entry: `packages/signals-royale-fx2-dalien` — engine at the root export, React
bindings at the `./react` subpath (consolidated from the former
`packages/react-signals-royale-fx2-dalien` in §14; earlier sections name the two
...
node royale/verify/count-loc.mjs --fork vendor/react --base e71a6393e66b0d2add46ba2b2c5db563a0563828 \
  --head royale/fx2-react --lib packages/signals-royale-fx2-dalien --lib packages/react-signals-royale-fx2-dalien
forkLoc: 11
libLoc: 2239
```
...
The two packages became one: `packages/react-signals-royale-fx2-dalien` is deleted
and the bindings live at `signals-royale-fx2-dalien/react`.
...
```
node royale/verify-kit/count-loc.mjs --lib packages/signals-royale-fx2-dalien
