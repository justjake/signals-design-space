...
settles). A write attributed to that still-live batch after the commit (a
...
and without the cap the render would fold that write mid-render — the same
...
model makes a yielded render's value change across a yield — exactly the
drift the contract forbids ("a render observes included-batch writes only
...
1. the render that mounted the component is the render now committing;
2. no committed-side advance happened since the render's pin;
...
2. **Check the committing batches' write clocks at commit time, not a
   ...
   the mask at render start*. A rendered batch whose **first** write lands
   mid-render interned its slot only after that capture, so "every
   ...
   model implements: at commit time, check the committing render's
   rendered **batches** themselves (latest write seq ≤ pin), so a
   member whose first write landed mid-render is seen.
...
3. **The conditions are only sound together with the per-batch
   ...
   write after an unrelated same-root render pinned. Every condition is
...
rendered.** A foreign batch retiring inside another render's commit would
...
enforces it as a schedule-legality rule in `renderEnd`.
