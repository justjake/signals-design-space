...
| RCC-RT5/6.alt-b-mount-world  | alt-b ruling (its suite #3): a component mounting inside a transition's commit reads the pending world in that same commit — same-commit agreement is what RT5/RT6 demand; which world the joint commit shows may differ | hold=sliced                                   | pass · skip (first commit tears under CPU-starved time slicing: the render-world note expires at a yield, later-slice readers resolve base; repair converges the next commit; both engines reproduce under CPU throttling ≥ 6×)  | mount probe log                 | implemented                                                                                        |
...
| DAISHI-8  | useDeferredValue: no tearing finally on mount                                                                                                 | deferred lattice, auto-inc          | pass · skip (same CPU-starved-slicing tear as RCC-RT5/6.alt-b-mount-world) | deferred lattice              | implemented |
