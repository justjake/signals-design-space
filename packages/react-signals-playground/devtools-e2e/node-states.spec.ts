...
test("errored node: arm-error surfaces an errored node with an errored-at time", async ({
  page,
}) => {
      ...
      page.evaluate(() =>
        (
          globalThis as unknown as {
            __SIGNALS_DEVTOOLS__: {
              search(q: string, n: number): Array<{ label?: string; status: string }>
            }
          }
        ).__SIGNALS_DEVTOOLS__
          .search("", 500)
...
test("suspended node: suspend-async surfaces a suspended node with a suspended-at time", async ({
  page,
}) => {
  await page.goto("/cosignals/?devtools")
  const panel = page.locator(".signals-devtools-root")
  ...
  const border = await panel
    .locator("g.node", { hasText: "asyncData" })
    .first()
    .evaluate((el) => {
    ...
    .poll(async () =>
      page.evaluate(
        () =>
          (
            globalThis as unknown as {
              __SIGNALS_DEVTOOLS__: { counts(): { byKind: { effect?: number } } }
            }
          ).__SIGNALS_DEVTOOLS__.counts().byKind.effect ?? 0,
      ),
    )
    .toBeGreaterThan(0)
})
