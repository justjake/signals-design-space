...
function holdWith(gate: Gate, gateAtom: WritableSignal<number>, writes: Record<string, unknown>): HoldHandle {
	openGate(gate);
...
function GateView(props: { gate: Gate; atom: ReadableSignal<number>; id: string }): React.ReactElement {
		...
		<section id="testpanel" aria-label="battery instrumentation">
			<div className="actions">
				...
				<button
					...
					onClick={() => startSignalTransition(() => atomOf('count').update((n) => (n as number) + 1))}
				>
					+1 transition
				</button>
				<button
			...
			<React.Suspense fallback={<span data-testid="gate-action-fallback">gate-action pending</span>}>
