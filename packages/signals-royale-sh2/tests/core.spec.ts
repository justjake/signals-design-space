...
import { atom, batch, computed, effect, initializeAtomState, read, serializeAtomState, set, update } from '../src';
...
describe('slab engine', () => {
  ...
  test('effects coalesce across a batch', () => {
    const value = atom(0);
    const seen: number[] = [];
    effect(() => { seen.push(read(value)); });
    batch(() => { set(value, 1); update(value, x => x + 1); });
