...
import { __debug, atom, computed, effect, retireBatch, subscribe, type BatchToken } from '../src/index.js';
...
function droppedEffect(source: ReturnType<typeof atom<number>>): WeakRef<object> {
	const dispose = effect(() => { source.read(); });
...
test('dropped effect and subscription disposers clean their retained graph links', async () => {
	...
	for (let attempt = 0; attempt < 50 && (source.subs.size !== 0 || __debug.listenerCount() !== 0); attempt++) {
		await new Promise(resolve => setTimeout(resolve, 0));
		globalThis.gc?.();
	}
	expect(source.subs.size).toBe(0);
