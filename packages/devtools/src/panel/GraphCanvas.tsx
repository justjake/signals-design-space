		...
		<div className="canvas-wrap">
			...
			{layout === undefined ? (
				...
				>
					...
					{layout.stubs.map((s, i) => (
						...
						<g key={i} className="stub" transform={`translate(${s.x},${s.y})`} role="button" tabIndex={0} onClick={onExpandColumn}>
			...
			{layout !== undefined && focusName !== undefined ? (
				<div className="canvas-status">
					drawn <b>{visNodes.length}</b> · set <b>{layout.shown}</b> of <b>{totalNodes}</b> · focus <b>{focusName}</b> · depth {depth} · pinch to zoom, drag or scroll to pan
				</div>
			) : undefined}
		</div>
	)
