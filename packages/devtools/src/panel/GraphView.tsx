...
export function GraphView({
  ...
  const [kindOn, setKindOn] = useState<Record<NodeKind, boolean>>({
    atom: true,
    computed: true,
    watcher: true,
    effect: true,
  ...
  const rows = allRows.filter(
    (n) => kindOn[n.kind] && (statusOnly === undefined || n.status === statusOnly),
  )
  ...
  const model = useMemo(
    () => (sel === undefined ? undefined : inspectorModel(backend, sel)),
    [backend, flush, sel],
  )
      ...
      <div className="toolbar">
        ...
        <div className="kind-filters" role="group" aria-label="Node kinds">
          {KIND_CHIPS.map((c) => (
            ...
            >
              <span className="kglyph" style={{ color: kindVar(c.kind) }}>
                {glyphFor(c.kind)}
              </span>
          ...
          >
            <span className="kglyph" style={{ color: "var(--danger)" }}>
              !
            </span>
            error · {errCount}
          </button>
          ...
          >
            <span className="kglyph" style={{ color: "var(--suspended)" }}>
              ⧗
            </span>
            suspended · {suspCount}
          </button>
