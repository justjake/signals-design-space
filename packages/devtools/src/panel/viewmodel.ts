...
function summarize(e: DevtoolsEvent): string {
  ...
  if (typeof d.next === "string")
    return typeof d.prev === "string" ? `${d.prev} → ${d.next}` : `→ ${d.next}`
  ...
  return {
    ...
    name:
      ...
      (typeof e.data.label === "string"
        ? e.data.label
        : typeof e.data.component === "string"
          ? e.data.component
          : undefined),
    summary: summarize(e),
    t: e.t,
 ...
 */
export function opGroups(rows: LogRow[]): {
  groups: Map<EventId, OpGroup>
  rootById: Map<EventId, EventId>
} {
  ...
  for (const r of rows) {
    ...
    if (g === undefined)
      groups.set(root, { minT: r.t, maxT: r.t, count: 1, renders: isRender ? 1 : 0, us: took })
    else {
      g.maxT = r.t
      g.count++
...
export function isUnstable(
  ...
  if (kind !== "computed" || sameResults !== 0 || newResults < 2 || preview === undefined)
    return false
  return (
    preview.startsWith("{") ||
...
export function inspectorModel(backend: Backend, id: NodeId): InspectorModel | undefined {
  ...
  if (node.status !== "ok") {
    const ev = backend.events(
      { node: id, classes: node.status === "error" ? ["error"] : ["async"] },
      1,
    )[0]
