          ...
          {mode === "flat"
            ? [...rows].reverse().map((r) => (
                <tr
                  ...
                  className={
                    `${r.id === selected ? "selected" : ""}${flashing.has(r.id) ? " flash" : ""}`.trim() ||
                    undefined
                  }
                ...
                >
                  ...
                  <td className="t">
                    {r.time}
                    {r.delta !== undefined ? (
                      <span className="tdelta"> {fmtDelta(r.delta)}</span>
                    ) : undefined}
                  </td>
                  <td>
                    <span className={`chip ${r.cls}`} data-tip={kindTip(r.kind)}>
                      {r.kind}
                    </span>
                  </td>
            ...
            : treeRows!.map((t) => (
                <tr
                  ...
                  className={
                    `${t.op % 2 === 1 ? "op-alt " : ""}${t.depth === 0 && t.children > 0 ? "op-head " : ""}${t.row.id === selected ? "selected " : ""}${flashing.has(t.row.id) ? "flash" : ""}`.trim() ||
                    undefined
                  }
                ...
                >
                  ...
                  <td className="t">
                    {t.row.time}
                    {t.row.delta !== undefined ? (
                      <span className="tdelta"> {fmtDelta(t.row.delta)}</span>
                    ) : undefined}
                  </td>
                  <td>
                    <span className={`chip ${t.row.cls}`} data-tip={kindTip(t.row.kind)}>
                      {t.row.kind}
                    </span>
                  </td>
          ...
          {rows.length === 0 ? (
            <tr>
              <td
                className="data"
                colSpan={6}
                style={{ color: "var(--faint)", fontStyle: "italic" }}
              >
                {emptyHint}
              </td>
            </tr>
