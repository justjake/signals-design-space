...
const ROLE: Record<Key, string> = {
	base00: 'background', base01: 'panels', base02: 'lines', base03: 'faint text',
	base04: 'muted text', base05: 'text', base06: 'bright', base07: 'brightest',
	base08: 'red · error', base09: 'orange', base0A: 'yellow · you', base0B: 'green · effect',
	base0C: 'cyan · compute', base0D: 'blue · async', base0E: 'purple · render', base0F: 'pink',
}
...
export function ThemeDialog({ open, onClose, root }: { open: boolean; onClose: () => void; root: HTMLElement | null }) {
	const dialogRef = useRef<HTMLDialogElement>(null)
	const [draft, setDraft] = useState<Record<Key, string>>(DEFAULT)
	const [note, setNote] = useState<{ text: string; kind: '' | 'ok' | 'err' }>({ text: '', kind: '' })
	...
	const onJson = (text: string) => {
		...
		const pal = (data as { palette?: Record<string, unknown> }).palette ?? (data as Record<string, unknown>)
		const next = { ...draft }
		let n = 0
		for (const k of KEYS) {
			const v = pal[k]
