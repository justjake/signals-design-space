...
function errorPreview(e: unknown): string {
  ...
  if (e && typeof e === "object" && "error" in e)
    return errorPreview((e as { error: unknown }).error)
  return preview(e)
}
...
function deepPreview(v: unknown, depth: number): string {
  ...
  const rows = keys
    .slice(0, 6)
    .map((k) => `  ${k}: ${indent(deepPreview((v as Record<string, unknown>)[k], depth - 1))},`)
...
function captureStack(): StackFrame[] {
  ...
  for (const line of raw.split("\n")) {
    ...
    if (m !== null)
      parsed.push({ fn: m[1] || "<anonymous>", file: m[2], line: Number(m[3]), col: Number(m[4]) })
  }
  ...
  for (const f of parsed) {
    ...
    if (
      /node_modules|cosignals|\/react-dom|\/react\/|\/react-jsx|\breact\.development\b|\/scheduler/.test(
        f.file,
      )
    )
      continue
...
function describeEvent(ev: Event): string {
  ...
  if (t instanceof Element) {
    ...
    if (t.id) where += `#${t.id}`
      ...
      where += `.${t.className.trim().split(/\s+/)[0]}`
...
export function createEngineDevtools(
  ...
  const provider: NodeProvider = {
    ...
    value(id) {
      ...
      return {
        preview: undefined,
        status: engine.nodeStatus(node) as NodeStatus,
        stale: false,
        pending: undefined,
      }
    },
    ...
    source(id) {
      ...
      if (kind === "effect") {
        ...
        return fn !== undefined
          ? `effect(${fn}${handler !== undefined ? `, ${handler}` : ""})`
          : undefined
      }
      return undefined
    },
    ...
    if (nodeIdNum !== undefined && (kind === "set" || kind === "update")) {
      ...
      if (parent === (0 as EventId)) {
        ...
        if (ev != null && typeof ev.type === "string") {
          ...
          if (key !== lastDomKey) {
            lastDomKey = key
            lastDomId = collector.record("dom-event", undefined, 0 as EventId, undefined, {
