/**
 * Page-world script. Attaches the collector to the page's fx2 engine (or
 ...
 * the app's fx2 module singleton via the trace-hook seam.
 *
 ...
 * fx2 build; fx2's trace hook is a module singleton, so one attach observes
 * all activity in that realm.
 */
import type { Backend } from '../protocol.ts'
import { attachFx2Devtools } from '../fx2.ts'
...
function start(): void {
	...
	const backend = existing ?? attachFx2Devtools().collector
