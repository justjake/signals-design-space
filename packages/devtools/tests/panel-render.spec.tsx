...
describe("inline host renders and live-updates from cosignals", () => {
  ...
  it('reveals the metric columns only after "+ metrics" is toggled on', async () => {
    ...
    try {
      ...
      const headers = () =>
        [...el.querySelectorAll(".nodelist thead th")].map((th) => th.textContent?.trim() ?? "")
      ...
      const row = [...el.querySelectorAll(".nodelist tbody tr")].find((r) =>
        r.textContent?.includes("doubled"),
      )!
      ...
      const runTimeTh = () =>
        [...el.querySelectorAll(".nodelist thead th")].find(
          (t) => t.textContent?.trim() === "run time",
        ) as HTMLElement
      await act(async () => {
        runTimeTh().click()
      })
