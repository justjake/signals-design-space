...
[`cosignals`](https://www.npmjs.com/package/cosignals)'s concurrent
...
- **Suspense in dep snapshots.** The model has no thenables, so battery
  ...
  (`cosignals/tests/suspense.spec.ts`) and at the React battery, not in
  lockstep.
...
- **Observation liveness.** Effect dep snapshots retain the RCC-OL1
  ...
  `cosignals/tests/observe-union.spec.ts` and the React suite.
