/**
 * The 17-case acceptance battery: the behavioral contract's required
 * outcomes (README.md states the contract) as deterministic named tests at
 * model level. The case numbers are stable identifiers shared with the
 * engine's own suites. Aspects that need a real React host are listed in
 * tests/SKIPPED-FOR-FORK-SUITE.md.
 *
 ...
 *      log entry retention, write storms, cycles, and node recreation
 ...
 *      (React updater-queue arithmetic: 2 then 4, never 3)
 *   4  two batches writing the same atom: the second still delivers, in
 *      its own lane
 *   5  delivery is value-blind; re-rendering re-arms it
 *   6  no implicit grouping: each write delivers in its own batch context
 *   7  writes and reads during a paused render (the pinned world never
 *      drifts; gap code reads newest)
 *   8  equality drops never lose log entries (only an empty-log no-op drops)
 *   9  mounting mid-transition (own render, foreign retirement, own-commit
 *      late write, stayed-live variant, hidden-then-revealed subtree)
...
describe('case 1 — world-divergent dependency (the killer; family)', () => {
	...
	it('core walk: k-world c=1 via the real k-dep; delivery in k lane; committed intact', () => {
		...
		expect(
			m.eventsOfType('delivery').filter((e) => e.watcher === 'W' && e.batch === k.id),
		).toHaveLength(1)
	...
	it('V2: write to the committed-only dep b in k over-notifies but never mis-values', () => {
		...
		// fold a=1 and b=9, so the dedup rule suppresses both follow-ups — a
		// scheduling decision, never an equality test on values.
		expect(
			m.eventsOfType('suppressed').filter((e) => e.watcher === 'W' && e.batch === k.id),
		).toHaveLength(2)
	...
	it('V6: slot reuse after k retires — tenancy orderings keep folds exact', () => {
		...
		// a fresh render including V folds k via the retired-history clause, then V via the included-up-to-pin clause, in seq order
	...
	it('taint member: untracked reads never notify and fold in-world', () => {
		...
		const c = m.computed(
			'c',
			(read, untracked) => (read(b) as number) + (untracked(a) as number) + 1,
		)
	...
	it('retention member: paused render across a foreign retirement (release rule at work)', () => {
		...
		m.renderResume(p.id)
		expect(m.renderValue(n, p)).toBe(15) // U retired after the pin (not history yet) and only T is included: a=5 → 15; U invisible
	...
	it('recreation member: changed deps mean a fresh node; evaluators never swap', () => {
		...
		expect(c1.fn).not.toBe(c2.fn) // no machinery anywhere swaps a live node's evaluating function
		selfCheck(m)
	})
})
...
describe('case 2 — flushSync excludes a pending default batch (why always-log)', () => {
	it('the excluding sync render sees BOTH old values; D folds later; watcher reconciles', () => {
		...
		expect(m.newestValue(a)).toBe(1) // the newest world applies writes eagerly
		...
		m.renderEnd(sync.id, 'commit')
		expect(
			m.eventsOfType('mount-corrective').filter((e) => e.watcher === 'W2' && e.batch === d.id),
		).toHaveLength(1)
...
describe('case 3 — rebase parity (React updater-queue arithmetic)', () => {
	it('replay-in-write-order over the pre-batch base: urgent commits 2, T commits 4 (never 3)', () => {
		...
		m.write(
			u.id,
			a,
			update((x) => (x as number) * 2),
		) // write log non-empty ⇒ always append
...
describe('case 6 — lane attribution under grouped notification', () => {
	it('no implicit grouping: each write delivers NOW in its own context', () => {
		...
		const urgent = m.openBatch() // the ambient event batch
		const transition = m.openBatch() // startTransition inside the same engine batch()
...
describe('case 7 — writes and reads during a yielded render pass', () => {
	it('yield-gap handlers read NEWEST and write into their own batch; the pinned world never drifts', () => {
		...
		m.renderYield(p.id) // fork renderYield ⇒ per-callstack truth: not-in-render
	...
	it('included-batch mid-render retirement: release blocked; clause 2 load-bearing; freed at render end', () => {
		...
		expect(m.renderValue(b, p)).toBe(7) // T retired after the pin, so the pre-pin write is admitted by inclusion (T in mask, written pre-pin)
		m.renderResume(p.id)
...
describe('case 9 — mount mid-transition (existing and fresh nodes)', () => {
	...
	it('(c) foreign retirement in the render→commit window fires the pre-paint correction', () => {
		...
		const a = m.atom('a', 0)
		const c = m.computed('c', (read) => (read(a) as number) + 1)
		const k = m.openBatch() // the transition being rendered
		const pk = openRender(m, 'A', [k])
		const w = m.mountWatcher(pk.id, c, 'W')
		expect(w.lastRenderedValue).toBe(1)
		m.renderYield(pk.id)
		...
		// value (which folds D's retired write) differs from the rendered value ⇒
		// urgent pre-paint correction
	...
	it('(d) own-commit fold of a post-pin included write falls through and fires', () => {
		...
		const fix = m.eventsOfType('mount-urgent-correction').filter((e) => e.watcher === 'W')
		expect(fix).toHaveLength(1) // k wrote after the pin ⇒ fast path falls through; the compare folds the late write
	...
	it("(d') if k stays live instead, the corrective loop covers it and no false urgent fires", () => {
		...
		expect(
			m.eventsOfType('mount-corrective').filter((e) => e.watcher === 'W' && e.batch === k.id),
		).toHaveLength(1)
		// Corrected expectation (recorded in tests/FLAGS.md under flag 5, finding 1):
		// an earlier design walk claimed that when k stays live, the corrective loop
		// alone covers it and the final compare comes out equal — no urgent
		// correction. But the commit's own lock-in precedes layout, so
		// committed-for-A already includes k's post-pin write via membership, and
		// the fast-forwarded world's committed clause (uncapped by the pin) folds
		// it: the compare fires — a value-TRUE urgent correction. Sound, because
		// committed truth really did move before paint; over-correcting toward
		// committed truth can never tear.
	...
	it('(e) reveal-shaped mount: render-pass-id conjunct fails, conservative compare corrects pre-paint', () => {
		...
		const u = m.openBatch() // one event writes a@s2 > p1 and reveals
		m.write(u.id, a, set(6))
		const pu = openRender(m, 'A', [u])
		m.adoptRevealedMount(pu.id, w.id) // W's layout effects fire inside u's commit
		m.renderEnd(pu.id, 'commit', { retireAtCommit: [u.id] })
		// the same-render condition FAILS (mounted by an older render) ⇒ conservative
		// fall-through ⇒ the fast-forwarded compare corrects pre-paint
		const fix = m.eventsOfType('mount-urgent-correction').filter((e) => e.watcher === 'W')
		expect(fix).toHaveLength(1)
		expect(w.lastRenderedValue).toBe(7)
...
describe('case 10 — late subscription joins the pending batch (entanglement)', () => {
	it("normal path: the corrective joins k's OWN lanes; exactly one commit", () => {
		...
		const urgent = openRender(m, 'A', []) // urgent render mounts W; its rendered world excludes k
	...
	it('race (i): k retires in the render→layout window — the w_fx compare corrects pre-paint', () => {
		...
		// advanced) ⇒ the fast-forwarded value folds k's retired write ≠ rendered
...
describe('case 11 — multiple roots (declared scope: degraded multi-root)', () => {
	it('per-root self-consistency; visible skew; exactly-once retirement; rows clear before release', () => {
		...
		const k = m.openBatch() // spans roots A and B
		m.write(k.id, a, set(1))
		...
		expect(
			m.eventsOfType('per-root-commit').filter((e) => e.root === 'A' && e.batch === k.id),
		).toHaveLength(1)
...
describe('case 14 — StrictMode and replayed renders (model-expressible half)', () => {
	...
	it('render-phase writes throw in all builds', () => {
		...
		expect(a.log).toHaveLength(0) // nothing landed
...
describe('case 16 — effects observe committed state only', () => {
	...
	it('16b — a committed-member write NEVER re-fires under an open same-root frame; the flip flushes at the frame close (kills immediate revalidation — codex finding 2)', () => {
		const m = concurrent()
		const a = m.atom('a', 0)
		const t = m.openBatch({ action: true }) // parked action: its batch stays live for member writes
		m.write(t.id, a, set(1))
