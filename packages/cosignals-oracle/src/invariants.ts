...
import { CosignalModel, InvariantViolation, type AtomNode, type World } from './model.js'
...
function worldName(w: World): string {
	switch (w.kind) {
		case 'newest':
			return 'newest'
		case 'render':
			return `render#${w.render.id}(pin ${w.render.pin})`
		case 'committed':
			return `committed(${w.root})`
		case 'mountFix':
			return 'mountFix'
	}
}
		...
		for (const atom of atoms(m)) {
			for (const e of atom.log) {
				...
				if (e.retiredSeq === undefined) {
					...
					if (tenant !== e.batch) {
						fail(
							`tenancy: un-retired log entry (seq ${e.seq}, batch ${e.batch}) bears slot ${slot.id} owned by ${String(tenant)}`,
						)
					}
					...
					if (e.retiredSeq >= slot.claimSeq) {
						fail(
							`tenancy: old tenant ${e.batch} retiredSeq ${e.retiredSeq} ≥ claim ${slot.claimSeq} of slot ${slot.id}`,
						)
					}
				}
				if (tenant === e.batch && e.seq <= slot.claimSeq) {
					...
					fail(
						`tenancy: tenant ${e.batch} log entry seq ${e.seq} ≤ claim ${slot.claimSeq} of slot ${slot.id}`,
					)
				}
			}
		}
		...
		for (const e of atom.log) {
			...
			if (batch.state === 'retired') {
				if (e.retiredSeq !== batch.retiredSeq) {
					fail(
						`retired batch ${batch.id} has log entry with retiredSeq ${String(e.retiredSeq)} ≠ batch stamp ${String(batch.retiredSeq)}`,
					)
				}
	...
	for (const w of relevantWorlds(m)) {
		for (const atom of atoms(m)) {
			...
			if (!Object.is(folded, shadow)) {
				fail(
					`retention: ${atom.name} in ${worldName(w)} folds ${String(folded)} but full history says ${String(shadow)}`,
				)
			}
		}
	}
	...
	for (const root of m.roots.values()) {
		for (const tid of root.committedBatches) {
			...
			if (t === undefined || t.state !== 'live') {
				fail(
					`per-root row for root ${root.id} names non-live batch ${tid} — per-root rows must clear at retirement`,
				)
			}
		}
	}
