 ...
 * arithmetic over those records. The kernel knows nothing about user
 * options: it compares values by reference identity only, has no error
 * handling of its own, and no async story.
 *
 * The policy layer around it — the Atom/ReducerAtom/Computed classes,
 * effect(), configure(), custom equality, error/suspension boxes — lives in
 * index.ts (the package entry, whose header carries the whole-package
 * vocabulary and reading order); the exception/suspension machinery the
 * kernel's two cold catch sites call into ({@link storeThrown},
 * {@link boxedRead}) lives in suspense.ts; the observed-lifecycle option's
 * state lives in lifecycle.ts.
 *
 * ## Storage
 *
 ...
 * columns {@link values}/{@link fns}, indexed by shifting the same
 * premultiplied id. Full definitions live in index.ts's header.
 *
 ...
 * Semantics are pinned by a 179-case conformance suite. Deviations from a
 * plain transliteration of upstream are enumerated in index.ts's header;
 * each deviation site here explains itself in place.
 *
...
import { lifecycleStates, releaseLifecycle, retainLifecycle } from './lifecycle.js';
 ...
 * hoisted policy functions (suspense.ts {@link ctxPrevious}/{@link ctxUse})
 * that resolve the evaluating node from the kernel's `activeSub`, so no
 * per-recompute state setup exists at all.
 */
...
// src/system.ts (SignalId/LinkId/SignalGen). One symbol carries every brand
// in the package (`IdBrand` below — the other layout-owning modules build
// their brands from it): brands under one key are exclusive by payload
// conflict, whereas brands under per-module symbols are silently mutually
// assignable (two different optional keys never conflict), which would void
// exactly the cross-layer protection this exists for. The plain aliases
// below the branded group are arithmetic-dominated values (flags, counters)
// whose every use is a mask/compare where a brand adds nothing.
 ...
 * ## Why exported?
 *
 * The declarations stay in this file — the kernel owns its record layout —
 * and the enums other modules walk kernel records with (NodeField,
 * LinkField, NodeFlag, ArenaShape) are exported so those consumers import
 * the one definition instead of hand-copying numbers a field reorder would
 * silently orphan. A cross-file member access still inlines under
 * whole-program tsc emit and esbuild bundling; per-file transforms (tsx,
 * vitest) fall back to a property read of the emitted enum object —
 * acceptable at the consumers' cold-to-warm sites, never in the kernel's
 * own hot paths (which are all same-file by construction).
 */
export const enum NodeField {
	 ...
	 * retain/release in {@link linkInsert}/{@link unlink} and lifecycle.ts's
	 * rehydration probe — atoms without the option never pay a lifecycle
	 * instruction.
	 *
	 * ## Why a whole field for one bit?
	 *
	 * We tried folding it into FLAGS as a bit. That forces {@link write} and
	 * {@link updateSignal} to preserve the bit, turning their constant flag
 ...
 * stride, and premultiplied ids with node records; see {@link NodeField} for
 * the offset-pointer access pattern and the const-enum rationale).
 */
...
export const enum NodeFlag {
	 ...
	 * (with {@link storeThrown}); the eval-start flag rewrite in
	 * {@link updateComputed} preserves the bits while the getter runs
	 * (ctx.previous and the isEqual wrapper filter the residual slot payload
	 * by them) and a successful evaluation clears them in the finally's flag
	 * write — every other flag site either ORs bits or is followed by a
	 * forced recompute (unwatched sets DIRTY), so a stale clear can never
	 * serve a payload unwrapped. */
	...
	/** Marks kernel records created by cosignals' own machinery (world folds,
	 * subscription captures) rather than by a user's handle. Its one hot job:
	 * keep machinery reads from counting toward a user atom's
	 * observed-lifecycle union — the "first subscriber attached / last one
	 * detached" callback ({@link linkInsert}/{@link unlink} skip the
	 * retain/release when the subscriber carries this bit; the machinery's
	 * observation index contributes to the union on its own terms instead, so
	 * a machinery computed's dep structure never pins an atom's remote
	 * subscription past its last real consumer). Set via the
	 * markMachineryOwned op when a computed gains concurrent-machinery
	 * content. */
	MACHINERY_OWNED  = 0b010000000000000,
}
 ...
 * {@link NodeField} for the const-enum rationale).
 */
 ...
 *
 *     E = createKernel(records * 2, E.buffer())
 *
 ...
 */
function createKernel(records: RecordCount, carry?: Int32Array): Kernel {
	...
	function freeNode(id: NodeId): void {
		...
		const v: ValueIndex = id >> ArenaShape.ID_TO_VALUE_SHIFT;
		vals[v] = undefined;
		vals[v + ArenaShape.AUX_VALUE_OFFSET] = undefined;
		fnTab[id >> ArenaShape.ID_TO_FN_SHIFT] = undefined;
	...
	function updateComputed(c: NodeId): boolean {
		...
		try {
			...
			// undefined).
			return oldValue !== (vals[v] = (fnTab[c >> ArenaShape.ID_TO_FN_SHIFT] as (ctx: unknown) => unknown)(evalCtx)) || oldExc !== 0;
		} catch (e) {
...
function boundaryWork(): void {
	...
	if (growPending) {
		...
		if (records !== E.records) {
			E = createKernel(records, E.buffer());
		}
	}
}
...
export function __resetKernelForTest(): void {
	...
	queued.length = 0;
	pendingFree.length = 0;
	// Side columns: stale values and wrapper/effect closures must not
	// survive id reuse.
	values.length = 2;
	values[0] = undefined;
	values[1] = undefined;
