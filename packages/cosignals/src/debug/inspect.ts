/**
 * `cosignals/debug/inspect` — read graph state WITHOUT perturbing it.
 *
 ...
 *
 * This module is inside cosignals, so it may read private node fields and know the
 ...
 * boundary that lets cosignals refactor internals freely.
 */
...
// cosignals nodes carry no id of their own; nodeOf maps a public Signal to its node.
 ...
 * out on first sighting). Pair with an engine id for global uniqueness. cosignals
 ...
 * write time) OR the lazy clock comparison cosignals uses internally — a dependency
