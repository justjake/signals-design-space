 ...
 * component's latest notification (or an effect's latest run) walks back to
...
export class Tracer {
  ...
  dropped = 0
  /** Most recent delivery event per node (component notification / effect run). */
  ...
  emit(
    ...
    const suspensionId = suspension === undefined ? undefined : this.suspensionIds.idFor(suspension)
  ...
  whyLastDelivery(node: ReactiveNode | object): string[] {
    ...
    if (start === undefined) {
      for (
        let link: Link | undefined = (node as ProducerNode).subs;
        link !== undefined;
        link = link.nextSub
      ) {
...
export function attachTracer(opts?: TracerOptions): Tracer {
  ...
  const causes = new WeakMap<object, TraceEventId>()
  const draftWrites = new WeakMap<object, TraceEventId>()
  setTracer({
    emitEvent: emit,
    startSpan: emit,
    getCause: (owner) => causes.get(owner) ?? NO_EVENT,
    setCause: (owner, cause) => causes.set(owner, cause),
    getDraftWrite: (draft) => draftWrites.get(draft) ?? NO_EVENT,
