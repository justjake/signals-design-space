...
export type AtomCtx<T> = { readonly state: T; set(value: T): void; update(fn: (current: T) => T): void };
...
export function createCosignals(options?: CreateCosignalsOptions) {
	...
	function __TEST__coreProbes(): { logEntries: number; batches: number; worldEvals: number; compositions: number } {
		return { ...probes };
	}
		...
		if (node !== undefined && node.kind === 'computed' && handle._internals === node) {
			...
			if (ws !== undefined) {
				for (const watcher of ws) {
					if (watcher.live) throw new ScheduleError(`disposeComputed(${node.name}): live watchers still subscribe — re-key them to the replacement first`);
				}
			}
	...
	function arenaLink(a: WorldArena, dep: number, sub: number, version: number, weak: boolean): number {
		const memory = a.memory;
	...
	function arenaLinkInsert(a: WorldArena, dep: number, sub: number, version: number, weak: boolean, prevDep: number, nextDep: number): number {
	...
	function shiftEffectDep(a: WorldArena, dep: ShadowId, delta: 1 | -1): void {
		...
		if (node !== undefined && a.memory[dep + ArenaField.NODE_GEN] === getKernelGeneration(node.id)) {
	...
	function ensureSignalEffectShadow(effect: SignalEffect): ShadowId {
		...
		if (a === undefined) throw new InvariantViolation(`SignalEffect ${effect.id} has no committed arena`);
		...
		} else if (a.memory[sh + ArenaField.NODE_GEN] !== gen || a.signalEffects[sh >> ArenaGeom.ID_TO_COLUMN_SHIFT] !== effect) {
			arenaEvictShadow(a, sh);
	...
	function runSignalEffectFrame(effect: SignalEffect, body: () => void): void {
		...
		a.memory[sh + ArenaField.DEPS_TAIL] = 0;
		a.memory[sh + ArenaField.FLAGS] = (a.memory[sh + ArenaField.FLAGS]! & ~(ArenaFlag.DIRTY | ArenaFlag.PENDING | ArenaFlag.RECURSED))
			| ArenaFlag.K_EFFECT | ArenaFlag.MUTABLE | ArenaFlag.VALID;
	...
	function signalEffectChanged(effect: SignalEffect): boolean {
		...
		while (link !== 0) {
			...
			const node = nodeIndexToInternals[ix];
			if (node !== undefined && a.memory[depShadow + ArenaField.NODE_GEN] === getKernelGeneration(node.id)) {
				const stamp = a.clocks[link >> ArenaGeom.ID_TO_COLUMN_SHIFT]!;
				const vi = depShadow >> ArenaGeom.ID_TO_COLUMN_SHIFT;
				if (!(
					(a.memory[depShadow + ArenaField.FLAGS]! & (ArenaFlag.VALID | ArenaFlag.DIRTY | ArenaFlag.PENDING | ArenaFlag.HAS_BOX)) === ArenaFlag.VALID
					&& a.clocks[vi] === stamp
	...
	function walkArenaStrong(a: WorldArena, from: NodeIndex, kGen: Generation, gen: WalkGen, found: Watcher[], effects: SignalEffect[]): void {
	...
	function queueNotify(kind: NotifyKind, obs: Watcher | undefined, t: Batch | undefined, slot: BatchSlot, effect?: SignalEffect): void {
	...
	class Watcher {
		...
		set live(value: boolean) {
			...
			memory[this.rec + ObserverField.FLAGS] = value ? flags | NodeFlag.OBSERVER_LIVE : flags & ~NodeFlag.OBSERVER_LIVE;
	...
	class SignalEffect {
		...
		constructor(id: SignalEffectId, name: string, root: RootId, nodes: (AnyInternals | undefined)[]) {
			this.id = id;
	...
	function mountSignalEffect(rootId: RootId, name: string): SignalEffect {
		...
		const effect = new SignalEffect(nextSignalEffectId++ as SignalEffectId, name, rootId, nodeIndexToInternals);
		root(rootId);
	...
	function clearResolvedDeliverySlots(effect: SignalEffect): void {
		...
		for (let slotId = 0; slotId < slots.length; slotId++) {
			...
			if (batch === undefined || batch.state === 'retired' || (tenant !== undefined && roots.get(effect.root)?.committedBatches.has(tenant) === true)) clear |= bit;
		}
	...
	function readSignalEffectDep(node: AnyInternals): Value {
		...
		if (effect === undefined) throw new ScheduleError('SignalEffect read requires an open execution frame');
	...
	function runCommittedSignalEffect(effect: SignalEffect): void {
		...
		effect.runs++;
		if (tr !== undefined) tr.reactEffectRun(effect.name, effect.root, effect.lastValue, effect.traceValues ?? []);
	}
	...
	function preserveDirtySignalEffects(rootFilter: RootId | undefined): void {
		...
		for (let i = 0; i < effects.length; i++) {
			...
			const a = effect.arena;
			const sh = effect.shadow;
			if (a !== undefined && sh !== 0 && (a.memory[sh + ArenaField.FLAGS]! & (ArenaFlag.DIRTY | ArenaFlag.PENDING)) === ArenaFlag.PENDING) {
	...
	function renderStart(rootId: RootId, includeBatches: BatchId[]): RenderPass {
		...
		const render: RenderPass = {
			...
			state: 'open', endKind: undefined, mounted: [], rendered: new Set(), signalEffects: [],
		};
	...
	function renderSignalEffect(renderPassId: RenderPassId, signalEffectId: SignalEffectId, willRun: boolean): void {
		const p = getRenderPassById(renderPassId);
		...
		if (effect === undefined || !effect.live) throw new ScheduleError(`unknown SignalEffect ${signalEffectId}`);
		if (effect.root !== p.root) throw new ScheduleError('signal effect rendered in a different root than its registration');
	...
	function assertIdleForReset(): void {
		...
		if (activeSignalEffect !== undefined) throw new ScheduleError('__TEST__resetEngine inside an open SignalEffect frame');
	...
	function __TEST__kernelSideColumns(id: NodeId): { value: unknown; aux: unknown; fn: Function | undefined } {
		const v: ValueIndex = id >> ArenaShape.ID_TO_VALUE_SHIFT;
	...
	function initializeAtomState(
		...
		if (!quiescent()) throw new ScheduleError('initializeAtomState requires no live batch or render');
	...
	composeEngine(options);
	return {
		Atom, ReducerAtom, Computed, effect, effectScope, batch, startBatch, endBatch, untracked, configure,
		serializeAtomState, initializeAtomState,
		SuspendedRead, attachDriver, engine, BATCH_NONE,
		__TEST__resetEngine, __TEST__coreProbes, __TEST__internalsById, __TEST__eachInternals,
		__TEST__kernelSideColumns, __TEST__ctxUse, __TEST__simulateReclaim, __TEST__reclaimStats,
		__TEST__useCache, __TEST__peekNextBatchId, __TEST__columns,
		get E() { return E; },
		get engineEpoch() { return engineEpoch; },
		maybeBoundary, getKernelGeneration, getKernelNodeIndex,
	};
}
