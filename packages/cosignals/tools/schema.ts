		...
		columns: [
			{
				...
				slots: [
					{ doc: 'current/computed value — watcher records: the last rendered value (subscriptions keep values in their extras object instead)' },
		...
		columns: [
			...
			},
			{
				...
				doc: [
					...
					're-checks, commit populator, and capture reads settle it against',
					'the cutoffVals register after their evaluations, and it moves only',
					'when the folded value changed since the last consult (see',
					'settleObserverClock: consult-driven on purpose — plain committed',
					'reads refold values but never move clocks, so observer re-fire',
					'behavior cannot depend on unrelated read timing). Root A\'s clocks',
					'never move on root B\'s commits because each root owns its arena;',
					'render arenas never settle (pin-frozen; the WorldArena.bumpsClocks',
					'gate). A link record\'s slot is observer state: on a subscription\'s',
					'dependency link it is that dep\'s lastValidatedAt — the producer',
					'clock the subscription last validated against (stamped at capture;',
					're-checks skip a clean producer whose clock still matches).',
					'Ordinary evaluation links never write it; the link free path scrubs',
					'it unconditionally. Zero means "never consulted" (the cutoff',
					'register\'s validity marker).',
				],
			},
			{
				name: 'cutoffVals',
				...
				scrubOnEvict: true,
				doc: [
					'The observer coalescing register (one slot per record; shadow slots',
					'only): the folded value as of the shadow\'s LAST OBSERVER CONSULT —',
					'the compare basis settleObserverClock moves the clock against. Kept',
					'separate from vals on purpose: plain committed reads refold vals',
					'freely between boundaries (committed-member writes are visible',
					'immediately), and if the clock compared against vals, an unrelated',
					'read could consume a flip-flop\'s intermediate state and change',
					'which re-fires observers see. Against this register, a multi-write',
					'flip-flop WITHIN one consult window coalesces to nothing and one',
