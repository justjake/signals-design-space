...
describe('ctx.use', () => {
	...
	test('the bare positional-factory form is gone (loud error), and keys reject objects/functions', () => {
		...
		expect(catchOf(() => c1.state)).toMatchObject({
			message: expect.stringMatching(/bare factory form.*removed/),
		})
		const c2 = new Computed((ctx) =>
			(ctx.use as (k: unknown, f: unknown) => unknown)({ q: 1 }, () => Promise.resolve(1)),
		)
		expect(catchOf(() => c2.state)).toMatchObject({
			message: expect.stringMatching(/keys must be strings, numbers/),
		})
		const c3 = new Computed((ctx) => (ctx.use as (s: unknown) => unknown)(42))
		expect(catchOf(() => c3.state)).toMatchObject({
			message: expect.stringMatching(/takes a thenable/),
		})
	})
