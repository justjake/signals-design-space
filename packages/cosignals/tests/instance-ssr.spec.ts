...
describe('createCosignals', () => {
	test('owns graph, values, effects, and policy state per instance', () => {
		...
		const seenA: number[] = [];
		const seenB: number[] = [];
		const stopA = one.effect(() => { seenA.push(ca.state); });
		const stopB = two.effect(() => { seenB.push(cb.state); });
	...
	test('serializes one request and initializes a separate hydration instance', () => {
		...
		count.set(7);
		const json = server.serializeAtomState(
			{ count, label },
			(key, value) => key === 'count' ? (value as number) + 1 : value,
		);

		const client = createCosignals();
		const clientCount = new client.Atom(0);
		const clientLabel = new client.Atom('client');
		client.initializeAtomState(
			json,
			{ count: clientCount, label: clientLabel },
			(key, value) => key === 'count' ? (value as number) - 1 : value,
		);
