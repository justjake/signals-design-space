...
function tevents<T extends TraceEvent['type']>(
	tr: Tracer,
	type: T,
): Extract<TraceEvent, { type: T }>[] {
	return decodedTraceEvents(tr).filter(
...
describe('R11 event-class coverage (staged narrative, one traced engine)', () => {
	...
	it('render lifecycle + mount fixup fast-out: render-start/render-end payloads, eval records, disposition', () => {
		...
		expect(evals.some((e) => e.data['node'] === 'c' && e.data['world'] === `render:${p1.id}`)).toBe(
			true,
		)
	...
	it('core effect run caused by its write; reconcile-correction caused by a top-level retirement', () => {
		...
		expect(tr.whyDelivered('W').map((e) => e.kind)).toEqual([
	...
	it('mount fixup: compare-clean when the fast-out fails but values agree', () => {
		...
		expect(last(tr, 'mount-fixup').data).toEqual({
			watcher: 'W4',
			root: 'A',
			disposition: 'compare-clean',
			correctives: 0,
		})
	})
	...
	it('20 seeds × 60 steps: session lossless; decode/format total; referee decode covers the stream; causality terminates', () => {
		for (let seed = 1; seed <= 20; seed++) {
			...
			const markers = decoded.filter(
				(e) => e.kind === 'render-committed' || e.kind === 'render-discarded',
			)
			...
			for (const m of markers) {
				expect(m.id, `seed ${seed}: marker #${m.id} must follow a render-end`).toBeGreaterThan(
					renderEnds.find(
						(e) =>
							e.data['renderPass'] === m.data['renderPass'] && e.data['root'] === m.data['root'],
					)!.id,
				)
			}
