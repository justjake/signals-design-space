 ...
 * 19  watchers map + nodeToWatchers index mutation     | dual store               | MUST MOVE TOGETHER via removeWatcher — FIXED (shim's map-only deletes stranded index entries) | T7 (new), cosignals-react graph-consumers.spec.tsx
 ...
 * 23  committed-subscription dep snapshots (captureRun) | committed VALUES + obs union | value-gated per EF2 boundary, no edge store; the capture's committed evaluations POPULATE the root's arena, whose marks the re-checks validate through (subDepRefs dissolved at S-B, §4.0) | concurrent-battery case 16, observe-union.spec, cosignals-react hooks.spec useSignalEffect
 *
