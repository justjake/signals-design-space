...
for (const p of probes) {
	...
	if (hasHoley(p.arr)) {
		console.error(
			`elements-kind probe: ${p.name} went HOLEY (length ${(p.arr as unknown[]).length})`,
		)
		failed = true
	}
	if (p.smi && !isSmi(p.arr)) {
		console.error(
			`elements-kind probe: ${p.name} left PACKED_SMI (length ${(p.arr as unknown[]).length})`,
		)
		failed = true
	}
}
