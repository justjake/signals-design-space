...
/**
 * Scenarios 11, 14-18, plus cosignals-specific surfaces (ambient transitions,
...
describe("scenario 15 — causality traces", () => {
  test("a suspension records observed render and readiness branches with shared identities", async () => {
    ...
    const ready = events.find((event) => event.kind === "retry" && event.cause === settled.id)!
  ...
  test("two sibling suspensions retain distinct identities when they settle in reverse", async () => {
    ...
    const secondReady = tracer
      .events()
      .find((event) => event.kind === "retry" && event.suspensionId === secondEvent.suspensionId)
    expect(secondReady).toBeDefined()
    expect(
      tracer
        .events()
        .some((event) => event.kind === "retry" && event.suspensionId === firstEvent.suspensionId),
    ).toBe(false)
  ...
  test("one suspension shared by two roots keeps one suspension id and two root ids", async () => {
    ...
    expect(
      tracer
        .events()
        .some((event) => event.kind === "retry" && event.suspensionId === suspensionId),
    ).toBe(true)
    tracer.stop()
  })
  ...
  test("replacing a tracer between a write and its delivery sanitizes the stale cause", async () => {
    ...
    expect(first.whyLastDelivery(nodeOf(value))).toEqual(["(no delivery recorded for this node)"])
  ...
  test("a transition roots at the ambient operation and its whole subtree chains to it", async () => {
    ...
    const events = tracer.events()
    const triggerWrite = events.find((event) => event.kind === "set" && event.label === "trigger")!
