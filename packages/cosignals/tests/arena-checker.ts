...
function stateFor(b: CosignalEngine): CheckerState {
	...
	if (st === undefined || st.epoch !== engineEpoch) {
		st = {
			engine: b,
			views: b.__TEST__checkerInternals(),
			epoch: engineEpoch,
			checking: false,
			naiveStack: new Set(),
		}
				...
				for (let nid = 0; nid < a.nodeToShadow.length; nid++) {
					...
					if (aDidThrow) {
						if (String(aThrew) !== String(mThrew)) {
							throw new InvariantViolation(
								`arena divergence: ${node.name} in ${a.kind} world of ${a.root}: arena threw ${String(aThrew)} but fold-truth threw ${String(mThrew)}`,
							)
						}
					} else {
						...
						const ceq =
							...
							!(aVal instanceof SuspendedRead) &&
							!(mVal instanceof SuspendedRead)
								? node.isEqual
								: undefined // sentinels compare by identity (16d), never through user comparators
						const same =
							ceq === undefined
								? Object.is(aVal!, mVal!)
...
function validateArena(v: ArenaCheckerInternals, a: WorldArena): void {
	...
	for (let nid = 0; nid < a.nodeToShadow.length; nid++) {
		...
		if ((flags & ArenaFlag.BOX_SUSPENDED) !== 0) {
			...
			throw new InvariantViolation(
				`arena ${a.root}: shadow ${sh} holds a suspended index without the bit`,
			)
		}
		if ((flags & ArenaFlag.DIRTY) !== 0 && !a.dirty.includes(sh)) {
			throw new InvariantViolation(
				`arena ${a.root}: DIRTY shadow ${sh} missing from the dirty list`,
			)
		}
		// deps list symmetry
