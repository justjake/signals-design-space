...
if (PART !== 'retain') {
	...
	t = await medianOfProcesses(
		`${DIR}/spkk1-logged.mjs`,
		{ EVENTS: 'truncate', DURATION_MS },
		PROCS,
		200_000,
	)
}
if (PART !== 'truncate') {
	...
	r = await medianOfProcesses(
		`${DIR}/spkk1-logged.mjs`,
		{ EVENTS: 'retain', DURATION_MS: RETAIN_MS },
		2,
		200_000,
	)
}
...
const out = {
	...
	'log at end (entries)': stat(g('logEnd'), 0),
