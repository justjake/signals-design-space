...
const node = bridge.adoptAtom
	...
	: (() => {
			const f = bridge.internalsForAtom ?? bridge.nodeForAtom
			const n = f.call(bridge, a)
			n.name = 'a'
			return n
		})()
...
if (probes.logEntries !== 0 || probes.batches !== 0) {
	throw new Error(
		`SPK-W quiet invariant: pipeline activity while quiet (${JSON.stringify(probes)})`,
	)
}
if (bridge.ambientBatch !== undefined) {
	throw new Error('SPK-W quiet invariant: ambient batch created')
}
