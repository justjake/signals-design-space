...
function untrackedFan() {
	...
	for (let r = 0; r < R; r++) {
		...
		const fans = Array.from({ length: K }, (_, i) =>
			b.computed(
				`c${r}.${i}`,
				(read, untracked) => Number(untracked(hot)) + Number(read(base)) + i,
			),
		)
...
console.log(
	`@@ROW ${JSON.stringify({ gate: 'S-B', shape: 'untracked-fan', metric: 'writeStormNsPerWrite', value: uf.ns, checksum: uf.checksum, root: ROOT })}`,
)
