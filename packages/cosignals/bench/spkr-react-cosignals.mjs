...
const { Atom } = mod
const { registerCosignalReact, useSignal } = await import(
	`${ROOT}/packages/cosignals-react/src/index.ts`
)
const { envInt, row } =
	await import('/Users/jitl/src/alien-signals-opt/packages/cosignals/bench/util.mjs')
...
const node = oldTree
	...
	: bridge.idToNode !== undefined
		? bridge.idToNode.get(a._id)
		: (mod.__TEST__internalsById ?? mod.__internalsByIdForTest)(a._id)
const checksum = container.textContent.length + Number(bridge.newestValue(node))
row({
	gate: 'SPK-R',
	config: 'react-cosignals',
	shape: `N${N}`,
	metric: `roundMs:N${N}`,
...
row({
	...
	config: 'react-cosignals',
	shape: `N${N}`,
	metric: `steadyLog:N${N}`,
	value: node.log.length + bridge.liveBatches().length,
	checksum,
})
handle.dispose?.()
