...
// (cosignals/trace) — records append inside the write as fixed-size ints
...
const tracer =
	b.events !== undefined
		? undefined
		: (await import(`${ROOT}/packages/cosignals/src/Tracer.ts`)).attachTracer(b, {
				mode: 'session',
			})
const deliveryMark = tracer === undefined ? () => b.events.length : () => tracer.stats().recorded
/** Invoke fn(watcherName, slotId) per delivery event recorded since `mark`. */
const eachDeliverySince =
	tracer === undefined
		? (mark, fn) => {
...
const base = {
	gate: 'SPK-N1',
	config: 'logged',
	shape: `F${F}xB${B}xW${W}${HELD ? '+held' : ''}${INTERLEAVE ? '+inter' : ''}`,
	checksum,
}
