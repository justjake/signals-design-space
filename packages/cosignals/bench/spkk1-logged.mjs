...
function takeSample(now) {
	...
	samples.push({
		...
		heap,
		k1: k1EdgeCount(),
		k1Keys: b.dependencyEdges.size,
		log: logTotal(),
		batches: b.idToBatch.size,
		renderPasses: b.idToRenderPass.size,
		events: eventsLen(),
		eventsRate: eventsSinceSample,
		writeNsMed: wMed,
	})
	eventsSinceSample = 0
	writeNsWindow = []
...
const slope =
	fit.reduce((s, p) => s + (p.t - mx) * (p.heap - my), 0) /
	fit.reduce((s, p) => s + (p.t - mx) ** 2, 0)
...
const base = {
	gate: 'SPK-K1',
	config: `logged-${EVENTS}`,
	shape: `soak${Math.round(DURATION_MS / 1000)}s`,
}
...
row({
	...base,
	metric: 'renderPassesPerHour',
	value: perHour('renderPasses'),
	checksum: end.renderPasses,
})
row({
	...base,
	metric: 'eventsPerHour',
	value: (samples.slice(2).reduce((s, p) => s + p.eventsRate, 0) / (end.t - start.t)) * 3600,
	checksum: 0,
})
row({ ...base, metric: 'framesPerSec', value: frame / end.t, checksum: frame })
console.log(
	`samples: ${JSON.stringify(samples.map((s) => ({ t: s.t, heapMB: +(s.heap / 1048576).toFixed(2), k1: s.k1, log: s.log, batches: s.batches, renderPasses: s.renderPasses, wNs: s.writeNsMed })))}`,
)
