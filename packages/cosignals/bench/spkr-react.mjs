...
// for N components reading one shared atom through cosignals-react, against
...
for (const N of [8, 64]) {
	...
	console.log(`== react-cosignals N${N} ==`)
	const c = await medianOfProcesses(`${DIR}/spkr-react-cosignals.mjs`, { N: String(N) }, PROCS)
	const uM = u.byMetric.get(`roundMs:N${N}`)
	const cM = c.byMetric.get(`roundMs:N${N}`)
	out.push({
		cell: `N${N}`,
		useStateMs: stat(uM, 3),
		cosignalsMs: stat(cM, 3),
