...
workloads.r1 = (iters) => {
	...
	for (let round = 0; round < 7; ++round) {
		dNs = Math.min(
			dNs,
			fastestOf(() => da.set(++x), iters, 2),
		)
		lNs = Math.min(
			lNs,
			fastestOf(() => la.set(++x), iters, 2),
		)
	}
...
workloads.r2 = (iters) => {
	...
	for (let round = 0; round < 7; ++round) {
		dNs = Math.min(
			dNs,
			fastestOf(() => dc.state, iters, 2),
		)
		lNs = Math.min(
			lNs,
			fastestOf(() => c.state, iters, 2),
		)
	}
	return { untraced: dNs, traced: lNs, ratio: lNs / dNs }
}
...
const iters = Number(
	process.argv[3] ??
		(name === 'deep'
			? 20000
			: name.startsWith('d2') || name.startsWith('w2') || name.startsWith('r2')
				? 200000
				: 50000),
)
...
if (typeof result === 'number') {
	...
	console.log(
		`${name}: untraced ${result.untraced.toFixed(1)}ns traced ${result.traced.toFixed(1)}ns ratio ${result.ratio.toFixed(3)}x`,
	)
}
