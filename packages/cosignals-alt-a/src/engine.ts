...
export function createCosignalEngine(options?: EngineOptions) {
	...
	const finalizer =
		finalizationEnabled && typeof FinalizationRegistry !== 'undefined'
	...
	function linkInsert(
		...
		if (loggedAtomCount !== 0) {
			...
			const producerMarked =
				(df & C.LOGGED) !== 0 || ((df & C.K_ATOM) === 0 && M[dep + C.OVERLAY_STAMP] > eraFloor)
			if (producerMarked) {
				const producerUnapplied =
					(df & C.K_ATOM) !== 0
	...
	function isLiveNode(id: number): boolean {
		return (
			liveCount[id >> 3] > 0 || (M[id + C.FLAGS] & (C.K_EFFECT | C.K_SCOPE | C.K_WATCHER)) !== 0
		)
	}
	...
	const COMMITTED_WORLD: WorldDesc = {
		k: C.WK_COMMITTED,
		key: -1,
		token: 0,
		slot: -1,
	...
	function visibleEntry(rec: number, world: WorldDesc): boolean {
		...
		switch (world.k) {
			...
			case C.WK_WRITER: {
				...
				return (
					(meta & C.M_PSEUDO) === 0 && (slot === world.slot || ((world.mask >> slot) & 1) !== 0)
				)
			}
		}
		return false
	...
	function overlayEvaluate(c: number, world: WorldDesc): unknown {
		...
		if (world.key >= 0) {
			...
			if (rec !== 0) {
				...
				if (world.k === C.WK_WRITER && world.slot >= 0 && W[rec + C.W_SLOT_NEXT] === -1) {
	...
	function resolveComputedInWorldInner(c: number, world: WorldDesc): unknown {
		...
		if (world.k === C.WK_NEWEST && (unappliedEntries === 0 || unappliedStamp[c >> 3] <= eraFloor)) {
	...
	function decide(w: number, token: number, entangled: boolean): void {
		...
		if (
			meta === undefined ||
			meta.watchedId === undefined ||
			(M[w + C.FLAGS] & C.K_WATCHER) === 0
		) {
	...
	function writeOp(a: number, op: number, payload: unknown): void {
		if (initDepth !== 0) {
			throw new Error(
				'cosignal: writes inside a lazy state initializer are not allowed (initializers must be pure)',
			)
		}
		...
		if (frameWorlds.length > 0 && frameWorlds[frameWorlds.length - 1].k === C.WK_PASS) {
		...
		if (forbidWritesInComputeds && canonicalEvalDepth > 0) {
			throw new Error(
				'cosignal: writes inside computeds are forbidden (configure.forbidWritesInComputeds, §12.5)',
			)
		}
	...
	function atom<T>(
		...
		values[v + 1] = initial
		if (
			opts?.isEqual !== undefined ||
			opts?.label !== undefined ||
			opts?.observeEffect !== undefined
		) {
			metas[id >> 3] = {
	...
	function verify(): void {
		...
		if (
			...
			for (const id of allNodes) {
				...
				if (
					(f & (C.K_COMPUTED | C.K_EFFECT | C.K_SCOPE | C.K_WATCHER)) !== 0 &&
					M[id + C.OVERLAY_STAMP] > eraFloor
				) {
	...
	return {
		...
		debug: {
			...
			quiescent: (): boolean => loggedAtomCount === 0 && passOpen === 0 && liveSlotMask === 0,
