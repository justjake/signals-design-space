...
describe('§18 gate measurements (reported; sanity ceilings only)', () => {
	it('G-6a: first logged write across cone sizes + streaming-store idle writes', () => {
		for (const cone of [10, 100, 1000]) {
			...
			report[`G-6a cone=${cone}`] = `DIRECT ${directNs.toFixed(0)}ns vs tape-create+retire ${createNs.toFixed(0)}ns = ${(createNs / directNs).toFixed(1)}x`;
		...
		report['G-6a streaming-store idle'] = `DIRECT ${directNs.toFixed(0)}ns vs LOGGED-idle ${idleNs.toFixed(0)}ns = ${(idleNs / directNs).toFixed(1)}x`;
	});
	...
	it('G-6b: steady logged urgent write (tape already exists) — spec target ≤2x', () => {
		...
		report['G-6b steady logged write'] = `DIRECT ${directNs.toFixed(0)}ns vs LOGGED ${steadyNs.toFixed(0)}ns = ${(steadyNs / directNs).toFixed(1)}x (spec gate 2x)`;
		holder.retire();
	...
	it('G-7: deferred write, drain-amortized, across watcher fan-outs — provisional ceiling 3x', () => {
		for (const fanout of [10, 100]) {
			...
			report[`G-7 fanout=${fanout}`] = `DIRECT-urgent ${directNs.toFixed(0)}ns vs deferred-drain ${deferredNs.toFixed(0)}ns = ${(deferredNs / directNs).toFixed(1)}x (provisional 3x)`;
			t.retire();
		}
	});
	...
	it('G-8: held-open transition, hot NEWEST read loop over the marked cone — spec gate 1.5x', () => {
		for (const certLen of [1, 4, 16]) {
			...
			report[`G-8 certLen=${certLen}`] = `DIRECT ${directNs.toFixed(0)}ns vs marked-NEWEST ${markedNs.toFixed(0)}ns = ${(markedNs / directNs).toFixed(1)}x (spec gate 1.5x)`;
			t.retire();
		}
	});
