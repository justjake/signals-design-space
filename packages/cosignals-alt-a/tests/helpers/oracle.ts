...
export function runSchedule(specs: NodeSpec[], ops: Op[], label: string): RunResult {
	...
	const atomHandles = new Map<
		number,
		{ set(v: number): void; update(f: (x: number) => number): void }
	>()
			...
			for (const tok of oracle.liveDeferredTokens()) {
				...
				if (!eqCompare(i, we, wo)) {
					return fail(
						step,
						op,
						`node ${i} writer(${tok}): engine=${String(we)} oracle=${String(wo)}`,
					)
				}
			}
			if (passExecuting()) {
				...
				for (const b of batches) {
					...
					if (!eqCompare(i, se, so)) {
						return fail(
							step,
							op,
							`node ${i} in-scope(${b.token}): engine=${String(se)} oracle=${String(so)}`,
						)
					}
				}
			}
	...
	function compareBroadcasts(step: number, op: Op, relevantTokens: number[]): RunResult {
		...
		if (as.join(';') !== es.join(';')) {
			return fail(step, op, `broadcast mismatch: engine=[${as.join(' ')}] oracle=[${es.join(' ')}]`)
		}
		return {}
	}
	...
	function resolveWriteToken(
		w: WriteDesc,
	): { token: number; deferred: boolean; script?: BatchScript } | undefined {
		if (w.batch === -1) {
		...
		try {
			switch (op.t) {
				...
				case 'write': {
					...
					const relevant = t.deferred ? [t.token] : [0, ...oracle.liveDeferredTokens()]
			...
			if (stray.length > 0) {
				return fail(
					step,
					op,
					`stray broadcasts: ${stray.map((x) => `${x.watcherId}:${x.token}`).join(',')}`,
				)
			}
...
export function generateUniverse(rng: () => number): NodeSpec[] {
	...
	for (let i = 0; i < nAtoms; ++i) {
		...
		if (roll < 0.25) {
			specs.push({
				kind: 'reducer',
				initial: Math.floor(rng() * 10),
				lazy: rng() < 0.3 ? true : undefined,
			})
		} else {
			specs.push({
				kind: 'atom',
	...
	for (let i = 0; i < length; ++i) {
		...
		if (roll < 0.32) {
			...
			if (openBatches < 6 && rng() < 0.5) {
				...
				const pick = (): number =>
					Math.floor(rng() * (idx + ops.filter((o) => o.t === 'newNode').length))
				ops.push({
					t: 'newNode',
					spec:
						rng() < 0.6
