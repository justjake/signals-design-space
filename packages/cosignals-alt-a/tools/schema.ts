...
export type FieldKind = 'flags' | 'NodeId' | 'LinkId' | 'LogId' | 'MemoId' | 'CertOff' | 'seq' | 'u31' | 'value' | 'spare';
...
export const schema: Schema = defineSchema({
	...
	planes: {
		...
		W: { stride: 8, doc: 'world-memo plane: overlay memo records (certificate region lives in a companion array; bulk-reset at quiescence)' },
	},
	records: [
		{
			name: 'node',
			plane: 'M',
			fields: [
				{ name: 'FLAGS', slot: 0, kind: 'flags', doc: 'state machine + kind bits', owner: 'alloc writes; free zeroes' },
				{ name: 'DEPS', slot: 1, kind: 'LinkId', doc: 'first link of my dependency list; doubles as free-list next for freed node records', owner: 'link/unlink; free threads' },
				{ name: 'DEPS_TAIL', slot: 2, kind: 'LinkId', doc: 'last confirmed dependency link (the re-run cursor)', owner: 'link/purgeDeps' },
				{ name: 'SUBS', slot: 3, kind: 'LinkId', doc: 'first link of my subscriber list', owner: 'linkInsert/unlink' },
				{ name: 'SUBS_TAIL', slot: 4, kind: 'LinkId', doc: 'last subscriber link', owner: 'linkInsert/unlink' },
				{ name: 'GEN', slot: 5, kind: 'u31', doc: 'generation counter, bumped on free; stale disposers no-op', owner: 'freeNode bumps' },
				{ name: 'LOG_HEAD', slot: 6, kind: 'LogId', doc: 'atoms: first log record id in plane G (0 = no log). Aliased as OVERLAY_STAMP on non-atoms.', owner: 'appendLog creates; sweep clears' },
				{ name: 'LOG_TAIL', slot: 7, kind: 'LogId', doc: 'atoms: last log record id. Aliased as MEMO_KEY on computeds.', owner: 'appendLog/sweep' },
			],
		},
		{
			name: 'link',
			plane: 'M',
			fields: [
				{ name: 'VERSION', slot: 0, kind: 'u31', doc: 'evaluation-cycle stamp: intra-run duplicate-read dedup', owner: 'link stamps' },
				...
				{ name: 'PREV_SUB', slot: 3, kind: 'LinkId', doc: "position in the producer's subscriber list", owner: 'linkInsert/unlink' },
				{ name: 'NEXT_SUB', slot: 4, kind: 'LinkId', doc: "position in the producer's subscriber list", owner: 'linkInsert/unlink' },
				{ name: 'PREV_DEP', slot: 5, kind: 'LinkId', doc: "position in the consumer's dependency list", owner: 'linkInsert/unlink' },
				{ name: 'NEXT_DEP', slot: 6, kind: 'LinkId', doc: "position in the consumer's dependency list; doubles as free-list next for freed link records", owner: 'linkInsert/unlink; free threads' },
			],
		},
		{
			name: 'log',
			plane: 'G',
			fields: [
				{ name: 'L_NEXT', slot: 0, kind: 'LogId', doc: "next entry in this atom's log (append order = seq order); 0 = tail; doubles as free-list next", owner: 'appendLog/sweep; free threads' },
				{ name: 'L_META', slot: 1, kind: 'flags', doc: 'packed: bits 0-1 OP, bit 2 APPLIED, bit 3 RETIRED, bits 4-8 BATCH_SLOT, bit 9 PSEUDO', owner: 'appendLog writes; retirement stamps RETIRED' },
				{ name: 'L_SEQ', slot: 2, kind: 'seq', doc: 'take-a-number ticket at append time', owner: 'appendLog/coalesce' },
				{ name: 'L_RETIRED_SEQ', slot: 3, kind: 'seq', doc: '0 while the batch is pending; one fresh ticket stamped per retirement', owner: 'retirement stamps' },
			],
		},
		{
			name: 'memo',
			plane: 'W',
			fields: [
				{ name: 'W_KEY', slot: 0, kind: 'u31', doc: 'world key: newest 0; pass (serial<<2)|1; writer (token<<2)|2', owner: 'overlayEvaluate' },
				{ name: 'W_EPOCH', slot: 1, kind: 'u31', doc: 'overlayEpoch at evaluation time; 0 is the tombstone value (epochs start at 1)', owner: 'overlayEvaluate; re-memoization tombstones' },
				{ name: 'W_NODE', slot: 2, kind: 'NodeId', doc: 'owning computed node id (drain re-validation + stale-head guard)', owner: 'overlayEvaluate' },
				{ name: 'W_VAL', slot: 3, kind: 'u31', doc: 'index into the memoVals side array holding the memoized value', owner: 'overlayEvaluate; tombstone clears the slot' },
				{ name: 'W_NEXT_MEMO', slot: 4, kind: 'MemoId', doc: "next memo record for the same node (the node's memo chain)", owner: 'overlayEvaluate prepends' },
				{ name: 'W_SLOT_NEXT', slot: 5, kind: 'MemoId', doc: "writer's-world records only: next record in the batch slot's memo chain; 0 on other keys", owner: 'overlayEvaluate; slot release clears heads' },
				{ name: 'W_NDEPS', slot: 6, kind: 'u31', doc: 'number of certificate pairs', owner: 'overlayEvaluate' },
				{ name: 'W_CERT', slot: 7, kind: 'CertOff', doc: "offset of this memo's certificate run in the certificate region", owner: 'overlayEvaluate' },
			],
		},
	],
	flags: [
		{ name: 'MUTABLE', value: 1, doc: 'can produce new values (atoms, computeds)' },
		{ name: 'WATCHING', value: 2, doc: 'wants notification when possibly stale (effects, watchers)' },
	...
	derivedMasks: [
		{ name: 'KIND_MASK', of: ['K_ATOM', 'K_COMPUTED', 'K_EFFECT', 'K_SCOPE', 'K_WATCHER'], doc: 'union of the kind bits; a freed record has FLAGS 0' },
	],
	groups: [
		{
			...
			members: [
				...
				{ name: 'OP_DISPATCH', value: 3, doc: "DISPATCH: the atom's reducer applies the stored action" },
				...
				{ name: 'M_PSEUDO', value: 512, doc: 'always-included pseudo-batch fallback (degrades toward urgent)' },
			],
		},
		{
			name: 'read contexts',
			...
			members: [
				...
				{ name: 'CTX_RENDER', value: 2, doc: 'while React executes render code: the pass world (Wp)' },
				{ name: 'CTX_COMMITTED', value: 3, doc: 'useSignalEffect callbacks and SSR: committed views' },
			],
		},
		{
			name: 'world kinds',
			...
			members: [
				{ name: 'WK_W0', value: 0, doc: 'the canonical world (committed + applied) the kernel maintains' },
				{ name: 'WK_NEWEST', value: 1, doc: 'every write visible' },
				{ name: 'WK_PASS', value: 2, doc: 'a render pass: pin + include mask' },
				{ name: 'WK_WRITER', value: 3, doc: "a batch's writer world: retired + applied + own entries" },
				{ name: 'WK_COMMITTED', value: 4, doc: 'committed views: retired-only, per-root refined by pin + lock-in mask' },
			],
		},
		{
			name: 'write modes',
			...
			members: [
				...
				{ name: 'MODE_LOGGED', value: 1, doc: 'every write is logged - permanently after first root registration' },
			],
		},
	],
	constants: [
		{ name: 'MAX_SEQ', value: 0x7fffffff, doc: 'infinity for pin comparisons' },
	],
	sideColumns: [
		{ name: 'values', index: 'id >> 2 (+1)', doc: 'atom current value / computed cached value; slot 1: atom kernel pending value / effect cleanup' },
