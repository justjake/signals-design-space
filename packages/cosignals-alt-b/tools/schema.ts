...
export const schema: Schema = defineSchema({
			...
			families: [
				{
					...
					fields: [
						{ name: 'FLAGS', slot: 0, kind: 'flags', doc: 'state machine + kind bits' },
						{
							...
							slot: 1,
							kind: 'LinkId',
							doc: 'first dependency link; free-list next when freed',
						},
						{
							name: 'DEPS_TAIL',
							slot: 2,
							kind: 'LinkId',
							doc: 'last confirmed dependency link (re-run cursor)',
						},
						...
						{ name: 'SUBS_TAIL', slot: 4, kind: 'LinkId', doc: 'last subscriber link' },
						{
							...
							slot: 5,
							kind: 'u31',
							doc: 'generation counter; bumped on free, defuses stale disposers',
						},
						{
							name: 'LOG_HEAD',
							slot: 6,
							kind: 'LogId',
							doc: 'atoms: first log record id in plane G (0 = no log)',
						},
						{
							name: 'OVERLAY_STAMP',
							slot: 6,
							kind: 'u31',
							alias: true,
							doc: 'computeds/effects/watchers: last notify-walk ticket; marked iff > eraFloor',
						},
						{ name: 'LOG_TAIL', slot: 7, kind: 'LogId', doc: 'atoms: last log record id' },
						{
							...
							kind: 'u31',
							alias: true,
							doc: 'computeds: world key of the head memo record (fast hit check)',
						},
					],
				},
				{
					...
					fields: [
						...
						{ name: 'SUB', slot: 2, kind: 'NodeId', doc: 'consumer node id' },
						{
							...
							slot: 3,
							kind: 'LinkId',
							doc: "previous link in the producer's subscriber list",
						},
						{
							name: 'NEXT_SUB',
							slot: 4,
							kind: 'LinkId',
							doc: "next link in the producer's subscriber list",
						},
						{
							name: 'PREV_DEP',
							slot: 5,
							kind: 'LinkId',
							doc: "previous link in the consumer's dependency list",
						},
						{
							name: 'NEXT_DEP',
							slot: 6,
							kind: 'LinkId',
							doc: "next link in the consumer's dependency list; free-list next when freed",
						},
					],
				},
			],
					...
					fields: [
						{
							...
							slot: 0,
							kind: 'LogId',
							doc: "next entry in this atom's log (append = seq order); 0 = tail; free-list next when freed",
						},
						{
							name: 'L_META',
							slot: 1,
							kind: 'packed',
							doc: 'packed: OP (bits 0-1), APPLIED (2), RETIRED (3), BATCH_SLOT (4-8), PSEUDO (9)',
						},
						{ name: 'L_SEQ', slot: 2, kind: 'u31', doc: 'take-a-number ticket at append time' },
						{
							...
							slot: 3,
							kind: 'u31',
							doc: '0 while pending; a fresh ticket stamped at retirement',
						},
					],
				},
			],
					...
					fields: [
						{
							...
							slot: 0,
							kind: 'u31',
							doc: 'world key (0 newest; (passSerial<<2)|1; (token<<2)|2)',
						},
						{
							name: 'W_EPOCH',
							slot: 1,
							kind: 'u31',
							doc: 'overlayEpoch at evaluation; 0 is the tombstone value',
						},
						{
							name: 'W_NODE',
							slot: 2,
							kind: 'NodeId',
							doc: 'owning computed (drain re-validation + stale-head guard)',
						},
						{
							name: 'W_VAL',
							slot: 3,
							kind: 'u31',
							doc: 'index into the memoVals side array (GC-visible value/box)',
						},
						{
							name: 'W_NEXT_MEMO',
							slot: 4,
							kind: 'MemoId',
							doc: "next memo record on the same node's chain",
						},
						{
							name: 'W_SLOT_NEXT',
							slot: 5,
							kind: 'MemoId',
							doc: "writer's-world records only: next record on the batch slot's chain",
						},
	...
	flags: [
		...
		{ name: 'PENDING', bit: 32, doc: 'possibly stale — verify by pulling before recomputing' },
		{
			...
			bit: 64,
			doc: 'dep list contains child effects/scopes (slow-path cleanup)',
		},
		{ name: 'LOGGED', bit: 128, doc: 'atoms only: LOG_HEAD !== 0 — the read gate' },
		{
			name: 'IMMEDIATE',
			bit: 256,
			doc: 'watchers only: notify via the broadcast list, not the effect queue',
		},
	...
	packedConsts: [
		...
		{ name: 'OP_DISPATCH', value: 3, doc: "DISPATCH: the atom's reducer applies the action" },
		{
			name: 'F_APPLIED',
			value: 4,
			doc: 'already written through the kernel (urgent writes, §9.4)',
		},
		...
		{ name: 'SLOT_MASK', value: 496, doc: '31 << SLOT_SHIFT' },
		{
			name: 'F_PSEUDO',
			value: 512,
			doc: 'slot-exhaustion fallback entry (§9.2), counted outside slots',
		},
	],
	namedConsts: [
		{
			name: 'REC_SLACK',
			value: 1280,
			doc: 'min free main-plane records guaranteed at each op boundary',
		},
	],
	budgets: {
