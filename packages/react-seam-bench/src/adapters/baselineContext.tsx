 ...
 */
import { createContext, startTransition, useContext, useEffect, useReducer, type ReactNode } from 'react';
...
const baselineContext: Contender = {
	...
	createCells(n) {
		...
		function requireDispatch(): (action: Action) => void {
			if (dispatchRef === null) throw new Error('baseline-context: write before the Provider mounted');
			return dispatchRef;
		}
		...
		return {
			...
			writeManyInTransition: (updates) => startTransition(() => requireDispatch()({ type: 'many', updates })),
			dispose() {
				dispatchRef = null;
			},
		};
