...
const SUBTITLE =
	process.argv[5] ??
	'react-seam-bench (jsdom, JS cost only): fanout write-to-commit median + transition urgent p95 + mount median, one process per contender - lower is better'
...
for (const [fw, e] of byFw) {
	if (e.tests < expectedTests) {
		partial.push(`${fw} (${e.tests}/${expectedTests} tests)`)
		continue
	}
...
for (let v = 0; v <= axisMax; v += step) {
	const gx = x(v)
	svg.push(
		`<line x1="${gx}" y1="${TOP - 8}" x2="${gx}" y2="${H - BOT + 4}" stroke="${GRID}" stroke-width="1"/>`,
	)
	svg.push(
		`<text x="${gx}" y="${H - BOT + 18}" font-size="11" fill="${INK2}" text-anchor="middle">${v.toLocaleString('en-US')}</text>`,
	)
}
svg.push(
	`<text x="${x(axisMax / 2)}" y="${H - 10}" font-size="11" fill="${INK2}" text-anchor="middle">total time (ms)</text>`,
)
...
frameworks.forEach((f, i) => {
	...
	SUITES.forEach((s, si) => {
		...
		if (isLast) {
			svg.push(
				`<path d="M ${cx} ${y} h ${Math.max(rw - 4, 0)} a 4 4 0 0 1 4 4 v ${BAR - 8} a 4 4 0 0 1 -4 4 h ${-Math.max(rw - 4, 0)} z" fill="${COLOR[s]}"/>`,
			)
		} else {
	...
	svg.push(
		`<text x="${cx + 8}" y="${y + BAR / 2 + 4}" font-size="12" fill="${INK2}">${Math.round(f.total).toLocaleString('en-US')} ms${i === 0 ? '' : `<tspan fill="#8a8984" font-size="11"> ${pctVsBest(f.total)}</tspan>`}</text>`,
	)
})
