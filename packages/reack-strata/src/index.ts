...
function represented(world: RenderWorld, branch: Branch, sequence: number): boolean {
	return sequence <= (world.cutoffs.get(branch) ?? 0) ||
...
function runSignalEffect(record: SignalEffectRecord): void {
	...
	if (record.disposed || record.fn === undefined || record.runtime === undefined || record.root === undefined) {
		return;
	}
...
export function useAtom<T>(
	...
	return React.useMemo(
		() => new Atom(initial as T | (() => T), options),
		[],
	);
}
