...
/**
 * In-package copy of the shared Signals Royale battery (royale/verify-kit),
 * pointed at this package's adapter: the real-React gate, scenarios 1-18.
 *
...
import adapter from '../royale/adapter.ts';
import type { HostHandle as RoyaleHandle } from '../src/host.ts';
...
function newRoot(): { root: { render(node: unknown): void; unmount(): void }; container: HTMLElement } {
...
function deferred<T>(): {
	...
	const d = { promise, resolve: (v: T) => { d.settled = true; resolve(v); }, settled: false };
	return d;
}
