/**
 * Cross-framework conformance: the full reactive-framework-test-suite runs
 * against the public engine API. `untracked` is implemented, so no section
 * skips.
 */
...
for (const { section, cases } of testSuite) {
	describe(`signals-royale-fh2 :: ${section}`, () => {
