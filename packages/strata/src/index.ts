...
function applyOperation<T>(operation: Operation<T>, value: T): T {
	return operation.kind === 0
		...
		: (operation.value as (value: T) => T)(value);
}
...
export class Runtime {
	...
	computed<T>(fn: (context: ComputedContext<T>) => T, options: ComputedOptions<T> = {}): Computed<T> {
	...
	finishBranch(branch: Branch, committed: boolean): void {
		...
		branch.retireCause = this.emitTrace(
			'batch-retire',
			branch,
			branch.lastCause || branch.cause,
			{ committed },
		);
	...
	private _fold<T>(
		...
		for (let i = 0; i < tape.length; i++) {
			...
			if (mode === 0) {
				...
				include = operation.branch === context ||
					!operation.branch.deferred || operation.branch.status === 1;
			} else {
	...
	_readWorld<T>(source: Source<T>, world: RenderWorld): T {
		...
		try {
			...
			if (result.error !== undefined) {
				...
				memo = source._hasStable && !world.deferred
	...
	isPending(source: Source): boolean {
		...
		if (this.activeWorld === undefined && this.activeConsumer === undefined && this.branchesByLane.size !== 0) {
			let lanes = 0;
			for (const branch of this.branchesByLane.values()) lanes |= branch.lane;
			const world = this.createWorld(this, lanes, this.noCutoffs, true, true, 0, true);
			try {
	...
	private _recompute<T>(computed: Computed<T>): void {
		...
		if (
			...
			const changed =
				!computed._hasValue || !computed.equals(computed._value as T, value as T);
			if (changed) {
		...
		const result = (this.evaluationFrames[evaluationDepth] ?? this.syncEvaluation) as
			AsyncEvaluation & { value?: T };
		this.evaluationFrames[evaluationDepth] = undefined;
		result.value = value;
...
export class ReducerAtom<S, A> extends Atom<S> {
	...
	constructor(reducer: (state: S, action: A) => S, initial: S | (() => S), options?: AtomOptions<S>, runtime?: Runtime) {
...
export class Computed<T = unknown> implements ConsumerState {
	...
	constructor(fn: (context: ComputedContext<T>) => T, options?: ComputedOptions<T>, runtime?: Runtime);
	constructor(
