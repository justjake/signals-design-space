...
function fold(
	atom: ModelAtom,
	include: (operation: ModelOperation) => boolean,
): number {
	let value = atom.base;
	...
	for (let step = 0; step < schedule.length; step++) {
		...
		if (operation.kind === 'set' || operation.kind === 'update') {
			...
			const next = operation.kind === 'set'
				? operation.value
				: updateFns[operation.update](previous);
			...
			if (!Object.is(previous, next)) {
				const modeled: ModelOperation = operation.kind === 'set'
		...
		const wantDynamic = (canonical(model[2]!) & 1) === 0
			? canonical(model[3]!)
			: canonical(model[4]!);
		...
		const latestDynamic = (newest(model[2]!) & 1) === 0
			? newest(model[3]!)
			: newest(model[4]!);
...
test('functional transition updates rebase over urgent updates', () => {
	...
	const urgent = runtime.activeBranches().next().value === transition
