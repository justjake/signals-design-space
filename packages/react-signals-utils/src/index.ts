...
export class ReactBatchRegistry {
	...
	constructor(react: unknown) {
		...
		this.consumer = {
			...
			onRenderPassStart: (root, container, lanes) => this.onRenderPassStart(root, container, lanes),
		...
		for (let i = 0; i < this.slots.length; i++) {
			...
			if (roots.size === 0) {
				;(retirements ??= []).push({
					slot,
					committed:
						visible || (slot.committedRoots !== undefined && slot.committedRoots.size !== 0),
				})
			} else if (visible) {
				;(slot.committedRoots ??= new Set()).add(root)
			}
