...
function batchesIn(lanes: number): Set<BatchToken> {
	...
	for (const token of tokens.values()) if (React.unstable_lanesInclude(lanes, token.lane)) found.add(token);
	return found;
}
...
function readForRender<T>(value: Value<T>): { value: T; root?: object; batches: Set<BatchToken> } {
	...
	return { value: withWorld(batches, context.container, () => read(value)), root: context.container, batches };
}
...
export { atom, batch, committed, computed, effect, initializeAtomState, isPending, latest, read, refresh, serializeAtomState, trace, untracked };
