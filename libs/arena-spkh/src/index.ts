/**
 * @lab/arena-spkh — SPK-H spike variant: the donor @lab/arena kernel with the
 * round-01 winner's TWO per-recompute policy hook sites added in their
 * cheapest honest form (synthesis §6: "LOGGED K0 gains exactly two
 * null-checked per-recompute callsites — beforeRetrack(n) (E-PRESERVE mirror
 * while forked) and afterRetrack(n) (flag OR-in) — never per-link, never
 * per-read"). The hooks are module-scalar function refs, null-checked at the
 * callsite and called only when armed. The DIRECT-mode measurement runs with
 * both hooks DISARMED (null) — the spike question is what the dormant
 * callsites cost. Everything below the SPK-H markers is the donor verbatim.
 *
 * Hook site placement (interpretation, documented in the findings note):
 * "per-recompute" = every dep-set-replacing re-evaluation, i.e. computed
 * recompute (updateComputed) AND effect re-run (run's dirty branch) — both
 * replace the node's dep set, which is exactly what E-PRESERVE's
 * beforeRetrack must observe. beforeRetrack fires before DEPS_TAIL is reset;
 * afterRetrack fires after the re-track completes (after purgeDeps). The
 * never-evaluated first-track paths (computedRead cold branch, newEffect) do
 * NOT get hook sites: they run once per node lifetime, not per recompute, and
 * the synthesis pins "exactly two per-recompute callsites".
 *
...
// ---- SPK-H: the winner's two per-recompute hooks (dormant in DIRECT) --------
// Module-level `let` scalars (NOT closure consts): LOGGED-mode arming must be
// observable by the live engine closure without a rebuild — that is the
// baseline design whose dormant cost this spike prices. Callsites null-check
// and call only when armed.
type RetrackHook = ((node: number) => void) | null;
let beforeRetrack: RetrackHook = null;
let afterRetrack: RetrackHook = null;

/** Arm/disarm the per-recompute hooks (LOGGED mode). Benchmarks never arm. */
export function setRetrackHooks(before: RetrackHook, after: RetrackHook): void {
	beforeRetrack = before;
	afterRetrack = after;
}
...
function createEngine(records: number, carry?: Int32Array): Engine {
	...
	function updateComputed(c: number): boolean {
		...
		try {
			...
			return oldValue !== (vals[v] = (fnTab[c >> 3] as (previousValue?: unknown) => unknown)(oldValue));
		} finally {
			--enterDepth;
			activeSub = prevSub;
