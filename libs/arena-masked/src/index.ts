...
function createEngine(
	...
	function allocNode(flags: number): number {
		...
		if (nodeFreeHead !== 0) {
			...
			id = nodeNext;
			if (id >= N.length) {
				throw new Error('@lab/arena-masked: node arena exhausted mid-operation; raise ARENA_MASKED_INITIAL_RECORDS');
			}
			nodeNext = id + 8;
			if (nodeNext > NODE_WM) {
				growPending = true;
	...
	function allocLink(): number {
		...
		if (linkFreeHead !== 0) {
			...
			id = linkNext;
			if (id >= L.length) {
				throw new Error('@lab/arena-masked: link arena exhausted mid-operation; raise ARENA_MASKED_INITIAL_RECORDS');
			}
			linkNext = id + 8;
			if (linkNext > LINK_WM) {
				growPending = true;
	...
	function link(dep: number, sub: number, version: number): void {
		...
		if (prevSub !== 0 && L[(prevSub + VERSION) & LMASK] === version && L[(prevSub + SUB) & LMASK] === sub) {
			return;
		}
	...
	function run(e: number): void {
		...
		if (
			flags & DIRTY
			|| (flags & PENDING && checkDirty(N[(e + DEPS) & NMASK], e))
		) {
...
const initialRecords = (() => {
	const env = (globalThis as { process?: { env?: Record<string, string | undefined> } })
		.process?.env?.ARENA_MASKED_INITIAL_RECORDS;
