...
if (mode === 'bench') {
	...
	for (const [name, shape] of Object.entries(SHAPES)) {
		...
		console.log(`@@ROW ${JSON.stringify({ lib: libName, shape: name, ms: +best.toFixed(2), checksum })}`);
	}
} else if (mode === 'prof') {
	...
	const ms = performance.now() - t0;
	console.log(`@@PROF ${JSON.stringify({ lib: libName, shape: shapeName, scale, ms: +ms.toFixed(1), checksum })}`);
} else {
