...
function run(name, build) {
	...
	console.log(
		`${name}: waves=${waves} visits=${v} perVisit{ flagLoads=${per(ctr.flagLoads)} flagStores=${per(ctr.flagStores)} flagRMWs=${per(ctr.flagRMWs)} linkLoads=${per(ctr.linkLoads)} nodeLoads=${per(ctr.nodeLoads)} stackPushes=${per(ctr.stackPushes)} notifies=${per(ctr.notifies)} }`,
	)
	dispose()
}
