...
function createEngine(
	...
	function allocNode(flags: number): number {
		...
		if (nodeFreeHead !== 0) {
			...
			if (id >= N.length) {
				throw new Error(
					'@lab/arena: node arena exhausted mid-operation; raise ARENA_PROBE_INITIAL_RECORDS',
				)
			}
			nodeNext = id + 8
			if (nodeNext > NODE_WM) {
	...
	function allocLink(): number {
		...
		if (linkFreeHead !== 0) {
			...
			if (id >= L.length) {
				throw new Error(
					'@lab/arena: link arena exhausted mid-operation; raise ARENA_PROBE_INITIAL_RECORDS',
				)
			}
			linkNext = id + 8
			if (linkNext > LINK_WM) {
	...
	function computedRead(c: number): unknown {
		...
		if (
			flags & DIRTY ||
			(flags & PENDING &&
				(checkDirty(N[c + DEPS], c) || ((N[c + FLAGS] = flags & ~PENDING), false)))
		) {
			if (updateComputed(c)) {
				const subs = N[c + SUBS]
				if (subs !== 0) {
