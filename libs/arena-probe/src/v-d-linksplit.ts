/**
 * VARIANT A+D — growth support OFF + link split for TurboFan inlining.
 * Same as v-a-nogrowth, plus: link() is split into a small fast path (the two
 * steady-state re-track hits) that fits V8's --max-inlined-bytecode-size=460
 * budget, and linkInsert() carrying the allocation tail. Measures the
 * link-non-inlining tax.
 */
