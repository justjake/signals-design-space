...
| **C3. `useComputed` evaluator swaps → deps-keyed node recreation** (useMemo semantics) | the evaluator staging/promotion/lineage-stamp apparatus (rounds 3–5's biggest bug factory) | closures re-create the node on deps change — the round-1 requirement's original reading |
| **C4. `ctx.previous` simplified or dropped for v1** | the three-way per-world previous rule and its memo obligations | an API nicety |
| **C5. Slot saturation → loud degrade** (throw/flag, not graceful spillover) | force-clear + spillover flags | graceful handling of >31 concurrent batches during one yielded transition (pathological) |
