...
You are the maintainer of packages/cosignals-alt-a in /Users/jitl/src/alien-signals-opt.
Question: **if the React fork (vendor/react, +5,012 lines over upstream base e71a6393e6,
19 files) only needed to support cosignals-alt-a and nothing else, how much could it
simplify? Goal: fewest fork LoC possible.** You may redesign alt-a in tandem.

Study: vendor/react patch surface (git diff e71a6393e6..HEAD -- packages), the bridge
(packages/cosignals-alt-a/src/react*.ts) for what is ACTUALLY consumed, the engine's
protocol needs (SPEC-RESOLUTIONS.md), and the RTL suite for behavioral requirements.

Deliver research/fork-simplification/alt-a-codex.md:
1. Inventory: every fork API/event alt-a consumes, with the feature it powers.
...
3. Tandem redesigns of alt-a that shrink the fork further (e.g. gate changes,
