...
function writeAtom(id: NodeId, isEqual: ((a: unknown, b: unknown) => boolean) | undefined, value: unknown): void {
	...
	maybeBoundary();
	// SPIKE seam: with live worlds, a changed kernel write also fans out into
	// each world's shadow subs (precise per-world invalidation). Zero worlds
	// pay exactly one scalar test.
	if (liveWorldCount !== 0) {
		const changed = values[(id >> Plane.ID_TO_VALUE_SHIFT) + Plane.AUX_VALUE_OFFSET] !== value;
		const propagated = E.write(id, value);
		if (changed) {
			worldFanout(id);
		}
		if (propagated && batchDepth === 0) {
			flush();
		}
		return;
	}
...
export class Atom<T> {
	...
	get state(): T {
		...
		// SPIKE seam: one predictable scalar test per read; armed only while a
		// world evaluation frame (or the red-mode naive read) is open.
		if (spikeRoute !== 0) {
			return spikeRoutedAtomRead(this as Atom<unknown>) as T;
		}
...
export class Computed<T> {
	 ...
	 */
	get state(): T {
		// SPIKE seam (the F5-unification read seam): one predictable scalar test
		// per computed read; armed only while a world evaluation frame is open.
		if (spikeRoute !== 0) {
			const v = spikeRoutedComputedRead(this as Computed<unknown>);
			if (v !== __HOST_MISS) {
				return v as T;
			}
		}
...
// ═══════════════════════════════════════════════════════════════════════════════
// SPIKE (NF2): world-tagged kernel links — per-world SEGREGATED shadow planes.
//
// Design choice (vs tags interleaved in shared lists): every speculative world
// gets its own plane of shadow records — shadow nodes mirror kernel nodes
// (field 5 = kernel id, field 6 = world id) and link records keep the exact
// kernel LinkField layout, allocated per world. World-0 (newest) lists are
// never entered by speculative links, so kernel sync walks stay byte-identical
// and per-world acyclicity is structural (a single evaluation stream per
// world), which is what makes the NF2 dispose walk terminate by construction.
// The walks below are transliterations of the kernel walks (M → w.W, no
// effect/notify branches — worlds have no effects; render pulls).
// ═══════════════════════════════════════════════════════════════════════════════

const enum WFlag {
	/** Shadow atom carries a world-local write; kernel write fanout skips it. */
	OVERRIDDEN = 8192,
}
...
class SpikeWorld {
	readonly id: number;
	alive = true;
	/** The world's shadow plane: node + link records, kernel stride/layout. */
	W: Int32Array;
	/** Side column: 2 slots per record (current value, pending/override value). */
	wvals: unknown[] = [undefined, undefined];
	next = Plane.STRIDE; // bump pointer (record 0 burned)
	linkFree = 0;
	wcycle = 0;
	/** Open shadow evaluation frame (shadow id of the evaluating computed). */
	frame = 0;
	...
	constructor(id: number, capInts: number) {
		this.id = id;
		this.W = new Int32Array(capInts);
	}
}
...
const liveWorlds: SpikeWorld[] = [];
const worldById = new Map<number, SpikeWorld>();
/** 0 = no spike routing; 1 = world evaluation open; 2 = naive (red) mode. */
let spikeRoute = 0;
let routedWorld: SpikeWorld | undefined = undefined;
let naiveWorld: SpikeWorld | undefined = undefined;

// World-walk scratch stacks (never shared with the kernel's).
let wPropStack = new Int32Array(4096);
let wPropSp = 0;
let wCheckStack = new Int32Array(4096);
let wCheckSp = 0;

function mustWorld(id: number): SpikeWorld {
	const w = worldById.get(id);
	if (w === undefined || !w.alive) {
		throw new Error(`cosignal spike: world ${id} is not live`);
	}
	return w;
}

function wGrow(w: SpikeWorld, need: number): void {
	let len = w.W.length;
	while (len < need) {
		len *= 2;
	}
...
function wAllocShadow(w: SpikeWorld, kernelId: NodeId, flags: NodeFlags): number {
	const id = w.next;
	wGrow(w, id + Plane.STRIDE);
	w.next = id + Plane.STRIDE;
	const W = w.W;
	W[id + NodeField.FLAGS] = flags;
	W[id + NodeField.GEN] = kernelId; // field 5: the kernel node this shadows
	W[id + NodeField.LIFECYCLE] = w.id; // field 6: the world tag
	const v: ValueIndex = id >> Plane.ID_TO_VALUE_SHIFT;
	while (w.wvals.length <= v + Plane.AUX_VALUE_OFFSET) {
		w.wvals.push(undefined);
	}
	w.ns.set(kernelId, id);
	return id;
}

function wAllocLink(w: SpikeWorld): number {
	let id = w.linkFree;
	if (id !== 0) {
		w.linkFree = w.W[id + LinkField.NEXT_DEP];
	} else {
		id = w.next;
		wGrow(w, id + Plane.STRIDE);
		w.next = id + Plane.STRIDE;
...
function wFreeLink(w: SpikeWorld, id: number): void {
	w.W[id + LinkField.NEXT_DEP] = w.linkFree;
	w.linkFree = id;
	w.links--;
}

// ---- transliterated link maintenance (M → w.W; world tag in field 7) ----------

function wLink(w: SpikeWorld, dep: number, sub: number, version: Version): void {
	const W = w.W;
	const prevDep = W[sub + NodeField.DEPS_TAIL];
	if (prevDep !== 0 && W[prevDep + LinkField.DEP] === dep) {
		return;
	}
	const nextDep = prevDep !== 0 ? W[prevDep + LinkField.NEXT_DEP] : W[sub + NodeField.DEPS];
	if (nextDep !== 0 && W[nextDep + LinkField.DEP] === dep) {
		W[nextDep + LinkField.VERSION] = version;
		W[sub + NodeField.DEPS_TAIL] = nextDep;
		return;
	}
	wLinkInsert(w, dep, sub, version, prevDep, nextDep);
}

function wLinkInsert(w: SpikeWorld, dep: number, sub: number, version: Version, prevDep: number, nextDep: number): void {
	const prevSub = w.W[dep + NodeField.SUBS_TAIL];
	if (prevSub !== 0 && w.W[prevSub + LinkField.VERSION] === version && w.W[prevSub + LinkField.SUB] === sub) {
		return;
	}
	const newLink = wAllocLink(w); // may grow the plane: reload W after
	const W = w.W;
	W[sub + NodeField.DEPS_TAIL] = newLink;
	W[dep + NodeField.SUBS_TAIL] = newLink;
	W[newLink + LinkField.VERSION] = version;
	W[newLink + LinkField.DEP] = dep;
	W[newLink + LinkField.SUB] = sub;
	W[newLink + LinkField.PREV_DEP] = prevDep;
	W[newLink + LinkField.NEXT_DEP] = nextDep;
	W[newLink + LinkField.PREV_SUB] = prevSub;
	W[newLink + LinkField.NEXT_SUB] = 0;
	W[newLink + 7] = w.id; // the world tag (spare field 7)
	if (nextDep !== 0) {
		W[nextDep + LinkField.PREV_DEP] = newLink;
	}
	if (prevDep !== 0) {
		W[prevDep + LinkField.NEXT_DEP] = newLink;
	} else {
		W[sub + NodeField.DEPS] = newLink;
	}
	if (prevSub !== 0) {
		W[prevSub + LinkField.NEXT_SUB] = newLink;
	} else {
		W[dep + NodeField.SUBS] = newLink;
	}
}

function wUnlink(w: SpikeWorld, id: number, sub: number = w.W[id + LinkField.SUB]): number {
	const W = w.W;
	const dep = W[id + LinkField.DEP];
	const prevDep = W[id + LinkField.PREV_DEP];
	const nextDep = W[id + LinkField.NEXT_DEP];
	const nextSub = W[id + LinkField.NEXT_SUB];
	const prevSub = W[id + LinkField.PREV_SUB];
	if (nextDep !== 0) {
		W[nextDep + LinkField.PREV_DEP] = prevDep;
	} else {
		W[sub + NodeField.DEPS_TAIL] = prevDep;
	}
	if (prevDep !== 0) {
		W[prevDep + LinkField.NEXT_DEP] = nextDep;
	} else {
		W[sub + NodeField.DEPS] = nextDep;
	}
	if (nextSub !== 0) {
		W[nextSub + LinkField.PREV_SUB] = prevSub;
	} else {
		W[dep + NodeField.SUBS_TAIL] = prevSub;
	}
	wFreeLink(w, id);
	if (prevSub !== 0) {
		W[prevSub + LinkField.NEXT_SUB] = nextSub;
	} else if ((W[dep + NodeField.SUBS] = nextSub) === 0 && (W[dep + NodeField.FLAGS] & NodeFlag.K_COMPUTED) !== 0) {
		// unwatched computed shadow: mark stale, tear down its own deps (in-world cascade)
		if (W[dep + NodeField.DEPS_TAIL] !== 0) {
			W[dep + NodeField.FLAGS] = NodeFlag.K_COMPUTED | NodeFlag.MUTABLE | NodeFlag.DIRTY;
			wDisposeAllDepsInReverse(w, dep);
		}
	}
	return nextDep;
}

function wDisposeAllDepsInReverse(w: SpikeWorld, sub: number): void {
	let cur = w.W[sub + NodeField.DEPS_TAIL];
	while (cur !== 0) {
		const prev = w.W[cur + LinkField.PREV_DEP];
		wUnlink(w, cur, sub);
		cur = prev;
	}
}

function wPurgeDeps(w: SpikeWorld, sub: number): void {
	const depsTail = w.W[sub + NodeField.DEPS_TAIL];
	let dep = depsTail !== 0 ? w.W[depsTail + LinkField.NEXT_DEP] : w.W[sub + NodeField.DEPS];
	while (dep !== 0) {
		dep = wUnlink(w, dep, sub);
	}
}

// ---- transliterated walks (no notify: worlds have no effects) -------------------

function wPropagate(w: SpikeWorld, startLink: number): void {
	const W = w.W; // never allocates: safe to cache
	let cur = startLink;
	let next = W[cur + LinkField.NEXT_SUB];
	const stackBase = wPropSp;
	top: do {
		const sub = W[cur + LinkField.SUB];
		let flags = W[sub + NodeField.FLAGS];
		if (!(flags & (NodeFlag.RECURSED_CHECK | NodeFlag.RECURSED | NodeFlag.DIRTY | NodeFlag.PENDING))) {
			W[sub + NodeField.FLAGS] = flags | NodeFlag.PENDING;
		} else if (!(flags & (NodeFlag.RECURSED_CHECK | NodeFlag.RECURSED))) {
			flags = 0;
		} else if (!(flags & NodeFlag.RECURSED_CHECK)) {
			W[sub + NodeField.FLAGS] = (flags & ~NodeFlag.RECURSED) | NodeFlag.PENDING;
		} else if (!(flags & (NodeFlag.DIRTY | NodeFlag.PENDING)) && wIsValidLink(w, cur, sub)) {
			W[sub + NodeField.FLAGS] = flags | (NodeFlag.RECURSED | NodeFlag.PENDING);
			flags &= NodeFlag.MUTABLE;
		} else {
			flags = 0;
		...
		if (flags & NodeFlag.MUTABLE) {
			const subSubs = W[sub + NodeField.SUBS];
			if (subSubs !== 0) {
				cur = subSubs;
				const nextSub = W[cur + LinkField.NEXT_SUB];
				if (nextSub !== 0) {
					if (wPropSp === wPropStack.length) {
						const bigger = new Int32Array(wPropStack.length * 2);
						bigger.set(wPropStack);
						wPropStack = bigger;
					}
					wPropStack[wPropSp++] = next;
					next = nextSub;
				}
				continue;
			}
		...
		if ((cur = next) !== 0) {
			next = W[cur + LinkField.NEXT_SUB];
			continue;
		}
		while (wPropSp > stackBase) {
			cur = wPropStack[--wPropSp];
			if (cur !== 0) {
				next = W[cur + LinkField.NEXT_SUB];
				continue top;
			}
...
function wShallowPropagate(w: SpikeWorld, startLink: number): void {
	const W = w.W;
	let cur = startLink;
	do {
		const sub = W[cur + LinkField.SUB];
		const flags = W[sub + NodeField.FLAGS];
		if ((flags & (NodeFlag.PENDING | NodeFlag.DIRTY)) === NodeFlag.PENDING) {
			W[sub + NodeField.FLAGS] = flags | NodeFlag.DIRTY;
		}
	} while ((cur = W[cur + LinkField.NEXT_SUB]) !== 0);
}

function wIsValidLink(w: SpikeWorld, checkLink: number, sub: number): boolean {
	const W = w.W;
	let cur = W[sub + NodeField.DEPS_TAIL];
	while (cur !== 0) {
		if (cur === checkLink) {
			return true;
		}
		cur = W[cur + LinkField.PREV_DEP];
	}
	return false;
}
...
function wCheckDirty(w: SpikeWorld, startLink: number, startSub: number): boolean {
	let cur = startLink;
	let sub = startSub;
	const stackBase = wCheckSp;
	let checkDepth = 0;
	let dirty = false;
	try {
		top: do {
			let W = w.W;
			const dep = W[cur + LinkField.DEP];
			const depFlags = W[dep + NodeField.FLAGS];
			if (W[sub + NodeField.FLAGS] & NodeFlag.DIRTY) {
				dirty = true;
			} else if ((depFlags & (NodeFlag.MUTABLE | NodeFlag.DIRTY)) === (NodeFlag.MUTABLE | NodeFlag.DIRTY)) {
				const depSubs = W[dep + NodeField.SUBS];
				if (wUpdate(w, dep)) {
					W = w.W;
					if (W[depSubs + LinkField.NEXT_SUB] !== 0) {
						wShallowPropagate(w, depSubs);
					}
					dirty = true;
				}
			} else if ((depFlags & (NodeFlag.MUTABLE | NodeFlag.PENDING)) === (NodeFlag.MUTABLE | NodeFlag.PENDING)) {
				if (wCheckSp === wCheckStack.length) {
					const bigger = new Int32Array(wCheckStack.length * 2);
					bigger.set(wCheckStack);
					wCheckStack = bigger;
				}
				wCheckStack[wCheckSp++] = cur;
				cur = W[dep + NodeField.DEPS];
				sub = dep;
				++checkDepth;
				continue;
			}
			if (!dirty) {
				const nextDep = w.W[cur + LinkField.NEXT_DEP];
				if (nextDep !== 0) {
					cur = nextDep;
					continue;
				}
			...
			while (checkDepth--) {
				cur = wCheckStack[--wCheckSp];
				if (dirty) {
					const subSubs = w.W[sub + NodeField.SUBS];
					if (wUpdate(w, sub)) {
						if (w.W[subSubs + LinkField.NEXT_SUB] !== 0) {
							wShallowPropagate(w, subSubs);
						}
						sub = w.W[cur + LinkField.SUB];
						continue;
					}
					dirty = false;
				} else {
					w.W[sub + NodeField.FLAGS] &= ~NodeFlag.PENDING;
				}
				sub = w.W[cur + LinkField.SUB];
				const nextDep = w.W[cur + LinkField.NEXT_DEP];
				if (nextDep !== 0) {
...
// ---- per-world evaluation (the kernel discipline, per (node, world)) ------------

function wUpdate(w: SpikeWorld, shadow: number): boolean {
	const flags = w.W[shadow + NodeField.FLAGS];
	if (flags & NodeFlag.K_COMPUTED) {
		return wUpdateComputed(w, shadow);
	}
	// shadow atom: fold override pending → current, or refresh from the kernel
	const v: ValueIndex = shadow >> Plane.ID_TO_VALUE_SHIFT;
	w.W[shadow + NodeField.FLAGS] = flags & ~(NodeFlag.DIRTY | NodeFlag.PENDING);
	if (flags & WFlag.OVERRIDDEN) {
		return w.wvals[v] !== (w.wvals[v] = w.wvals[v + Plane.AUX_VALUE_OFFSET]);
	}
	return w.wvals[v] !== (w.wvals[v] = E.read(w.W[shadow + NodeField.GEN]));
}

function wUpdateComputed(w: SpikeWorld, shadow: number): boolean {
	w.W[shadow + NodeField.DEPS_TAIL] = 0;
	w.W[shadow + NodeField.FLAGS] = NodeFlag.K_COMPUTED | NodeFlag.MUTABLE | NodeFlag.RECURSED_CHECK;
	const kid = w.W[shadow + NodeField.GEN];
	const prevRoute = spikeRoute;
	const prevRouted = routedWorld;
	const prevFrame = w.frame;
	spikeRoute = 1;
	routedWorld = w;
	w.frame = shadow;
	const v: ValueIndex = shadow >> Plane.ID_TO_VALUE_SHIFT;
	const oldValue = w.wvals[v];
	try {
		++w.wcycle;
		return oldValue !== (w.wvals[v] = (fns[kid >> Plane.ID_TO_FN_SHIFT] as (ctx: unknown) => unknown)(POLICY_CTX));
	} finally {
		spikeRoute = prevRoute;
		routedWorld = prevRouted;
		w.frame = prevFrame;
		w.W[shadow + NodeField.FLAGS] &= ~NodeFlag.RECURSED_CHECK;
		wPurgeDeps(w, shadow);
	}
}

function wAtomRead(w: SpikeWorld, handle: Atom<unknown>): unknown {
	w.readClock++;
	const kid = handle._id;
	let shadow = w.ns.get(kid);
	if (shadow === undefined) {
		shadow = wAllocShadow(w, kid, NodeFlag.K_SIGNAL | NodeFlag.MUTABLE);
	}
	if (w.W[shadow + NodeField.FLAGS] & NodeFlag.DIRTY) {
		if (wUpdate(w, shadow)) {
			const subs = w.W[shadow + NodeField.SUBS];
			if (subs !== 0) {
				wShallowPropagate(w, subs);
			}
		}
	}
	const f = w.frame;
	if (f !== 0) {
		wLink(w, shadow, f, w.wcycle);
	}
	const v: ValueIndex = shadow >> Plane.ID_TO_VALUE_SHIFT;
	if (w.W[shadow + NodeField.FLAGS] & WFlag.OVERRIDDEN) {
		return w.wvals[v];
	}
	return (w.wvals[v] = E.read(kid)); // non-overridden shadows track the kernel live
}

function wComputedRead(w: SpikeWorld, handle: Computed<unknown>): unknown {
	w.readClock++;
	const kid = handle._id;
	let shadow = w.ns.get(kid);
	if (shadow === undefined) {
		shadow = wAllocShadow(w, kid, NodeFlag.K_COMPUTED);
	}
	const flags = w.W[shadow + NodeField.FLAGS];
	if (flags & NodeFlag.RECURSED_CHECK) {
		throw new CycleError('cosignal: world computed read during its own evaluation (dependency cycle).');
	}
	if (
		flags & NodeFlag.DIRTY
		|| (
			flags & NodeFlag.PENDING
			&& (
				wCheckDirty(w, w.W[shadow + NodeField.DEPS], shadow)
				|| (w.W[shadow + NodeField.FLAGS] &= ~NodeFlag.PENDING, false)
			)
		)
	) {
		if (wUpdateComputed(w, shadow)) {
			const subs = w.W[shadow + NodeField.SUBS];
			if (subs !== 0) {
				wShallowPropagate(w, subs);
			}
		}
	} else if (flags === NodeFlag.K_COMPUTED) {
		wUpdateComputed(w, shadow); // never evaluated in this world
	}
	const f = w.frame;
	if (f !== 0) {
		wLink(w, shadow, f, w.wcycle);
	}
	return w.wvals[shadow >> Plane.ID_TO_VALUE_SHIFT];
}

// ---- spike routing dispatch (cold; reached only when spikeRoute !== 0) ----------

function spikeRoutedAtomRead(handle: Atom<unknown>): unknown {
	if (spikeRoute === 1) {
		return wAtomRead(routedWorld!, handle);
	}
	// naive (red) mode: the kernel read runs (links form under the evaluating
	// kernel computed) and the world's override REPLACES the result — world
	// folds ride the kernel's own records: the failed design reproduced.
	const value = E.read(handle._id);
	const w = naiveWorld!;
	const shadow = w.ns.get(handle._id);
	if (shadow !== undefined && (w.W[shadow + NodeField.FLAGS] & WFlag.OVERRIDDEN) !== 0) {
		return w.wvals[(shadow >> Plane.ID_TO_VALUE_SHIFT) + Plane.AUX_VALUE_OFFSET];
	}
	return value;
}

function spikeRoutedComputedRead(handle: Computed<unknown>): unknown {
	if (activeSub !== 0 && spikeRoute === 1) {
		// world routing never intercepts an open KERNEL frame (same rule as the
		// shipped hostRead seam) — unreachable in practice: worlds open no kernel frames.
		return __HOST_MISS;
	}
	if (spikeRoute === 1) {
		return wComputedRead(routedWorld!, handle);
	}
	// naive (red) mode: per-world memo semantics grafted onto the kernel node —
	// force a re-track through the kernel's own record on every world read.
	E.invalidateComputed(handle._id);
	return __HOST_MISS; // fall through to the kernel computedRead
}

// ---- kernel write fanout into live worlds ---------------------------------------

function worldFanout(kid: NodeId): void {
	for (let i = 0; i < liveWorlds.length; i++) {
		const w = liveWorlds[i];
		const shadow = w.ns.get(kid);
		if (shadow === undefined) {
			continue;
		}
		const flags = w.W[shadow + NodeField.FLAGS];
		if (flags & WFlag.OVERRIDDEN) {
			continue; // the world's own write wins; kernel motion invisible
		}
		if ((flags & NodeFlag.DIRTY) !== 0 && w.W[shadow + 7] === w.readClock) {
			continue; // cone already marked and nothing re-validated since (idle-world storm dedup)
		}
		w.W[shadow + NodeField.FLAGS] = flags | NodeFlag.DIRTY;
		w.W[shadow + 7] = w.readClock; // field 7 spare on shadow node records
		const subs = w.W[shadow + NodeField.SUBS];
		if (subs !== 0) {
			wPropagate(w, subs);
		}
...
// ---- spike public surface --------------------------------------------------------

/** Opens a speculative world (an independent evaluation stream). @internal spike */
export function __worldBegin(): number {
	const w = new SpikeWorld(++worldSeq, 8192);
	liveWorlds.push(w);
	worldById.set(w.id, w);
	liveWorldCount = liveWorlds.length;
	return w.id;
}

/** World-local write: invalidates within `w` only. @internal spike */
export function __worldSet(id: number, atom: Atom<unknown>, value: unknown): void {
	...
	const v: ValueIndex = shadow >> Plane.ID_TO_VALUE_SHIFT;
	w.wvals[v + Plane.AUX_VALUE_OFFSET] = value;
	w.W[shadow + NodeField.FLAGS] |= WFlag.OVERRIDDEN | NodeFlag.DIRTY;
	const subs = w.W[shadow + NodeField.SUBS];
	if (subs !== 0) {
		wPropagate(w, subs);
	}
...
export function __worldRead(id: number, sig: Atom<unknown> | Computed<unknown>): unknown {
	const w = mustWorld(id);
	if (activeSub !== 0) {
		throw new Error('cosignal spike: world reads may not run inside a kernel evaluation frame.');
	}
	if (sig instanceof Computed) {
		return wComputedRead(w, sig);
	}
...
export function __worldDiscard(id: number, mode: 'surgical' | 'bulk' = 'bulk'): void {
	const w = mustWorld(id);
	if (mode === 'surgical') {
		for (const shadow of w.ns.values()) {
			if (w.W[shadow + NodeField.FLAGS] & NodeFlag.K_COMPUTED) {
				wDisposeAllDepsInReverse(w, shadow);
			}
		}
		if (w.links !== 0) {
			throw new Error(`cosignal spike: surgical discard leaked ${w.links} links`);
		}
	}
	w.alive = false;
	worldById.delete(id);
	const i = liveWorlds.indexOf(w);
	if (i >= 0) {
		liveWorlds.splice(i, 1);
	}
	liveWorldCount = liveWorlds.length;
	w.ns.clear();
	w.wvals.length = 0;
...
/** @internal spike */
export function __worldLiveCount(): number {
	return liveWorldCount;
}

/** @internal spike */
export function __worldStats(id: number): { links: number; shadows: number; planeBytes: number } {
	const w = mustWorld(id);
...
export function __naiveWorldRead(id: number, sig: Atom<unknown> | Computed<unknown>): unknown {
	const w = mustWorld(id);
	const prevRoute = spikeRoute;
	const prevNaive = naiveWorld;
	spikeRoute = 2;
	naiveWorld = w;
	try {
		if (sig instanceof Computed) {
			E.invalidateComputed(sig._id);
			return E.computedRead(sig._id);
		}
		return spikeRoutedAtomRead(sig as Atom<unknown>);
	} finally {
		spikeRoute = prevRoute;
		naiveWorld = prevNaive;
	}
}
...
export function __spikeGraphCheck(ids: number[]): { links: number } {
	...
	let total = 0;
	for (const id of ids) {
		let cur = M[id + NodeField.DEPS];
		let prev = 0;
		let steps = 0;
		while (cur !== 0) {
			if (++steps > CAP) throw new Error(`spike graph check: deps list of node ${id} exceeds ${CAP} steps (cycle)`);
			if (M[cur + LinkField.SUB] !== id) throw new Error(`spike graph check: link ${cur} SUB != owner ${id}`);
			if (M[cur + LinkField.PREV_DEP] !== prev) throw new Error(`spike graph check: link ${cur} PREV_DEP broken`);
			const dep = M[cur + LinkField.DEP];
			let s = M[dep + NodeField.SUBS];
			let ssteps = 0;
			let found = false;
			while (s !== 0) {
				if (++ssteps > CAP) throw new Error(`spike graph check: subs list of node ${dep} exceeds ${CAP} steps (cycle)`);
				if (s === cur) {
					found = true;
					break;
				}
				s = M[s + LinkField.NEXT_SUB];
			}
			if (!found) throw new Error(`spike graph check: link ${cur} missing from dep ${dep} subs list (asymmetry)`);
			prev = cur;
			cur = M[cur + LinkField.NEXT_DEP];
			total++;
		}
		let scur = M[id + NodeField.SUBS];
		let sprev = 0;
		let s2 = 0;
		while (scur !== 0) {
			if (++s2 > CAP) throw new Error(`spike graph check: subs list of node ${id} exceeds ${CAP} steps (cycle)`);
			if (M[scur + LinkField.DEP] !== id) throw new Error(`spike graph check: link ${scur} DEP != owner ${id}`);
			if (M[scur + LinkField.PREV_SUB] !== sprev) throw new Error(`spike graph check: link ${scur} PREV_SUB broken`);
			sprev = scur;
			scur = M[scur + LinkField.NEXT_SUB];
		}
	}
	return { links: total };
}
