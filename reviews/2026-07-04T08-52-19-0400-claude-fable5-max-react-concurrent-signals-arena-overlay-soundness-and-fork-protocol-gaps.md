...
Design time is the only cheap moment to change a packed layout, so every fix was evaluated for whether it *wants* new packed state, not merely whether it can avoid it. Outcome: two layout additions are worth designing in now — a packed **world-memo plane `W`** (serves F2+F3, and is the difference between meeting and missing G-8) and a **token column on the broadcast queue** (F6) — while the `M`/`G` record layouts, strides, and the flags word survive all fixes unchanged, a conclusion the integration section reaches by pricing the alternatives (including node stride 16), not by assumption. Two fixes (F1, F6) change the fork protocol / M0 deliverable and one (F2+F3) changes M3's data structures — these are the ones that are expensive to discover later, because the milestones build on top of them.
...
**Fix (memo record shape — packed; full plane design in the integration section):**
...
Certificates get longer (all reads, not just logged ones) — which is the strongest argument for packing them with the memo record (plane `W`, integration section) rather than growing heap arrays: validation sits on the G-8 hot loop and becomes one contiguous Int32 scan instead of three dependent heap loads into scattered arrays; and the collector becomes a persistent scratch `Int32Array` with saved frame bases (the `propStack` discipline), which is exactly the structure that turns the nested-frame flattening of point 2 into a base-index rule instead of an allocation.
...
## Holistic integration: the layout, revisited at design time

Design time is the only cheap moment to change a packed layout, so every fix was evaluated three ways: does it want new per-node fields in plane `M`? does it want its own packed structure? or is side/heap state genuinely the right home (cold, per-root, or GC-visible by nature)? Two layout additions earn their place; one node-record change was priced and rejected.

### Adopted: plane `W` — a packed world-memo plane (serves F2 + F3, and gate G-8 directly)

World memos are heap objects in the current spec (`{key, epoch, value, srcAtoms[], srcSeqs[]}` in the `memos` side column; "one small object (~48–80 B) … per overlay *evaluation*", 7.6). The fixes change their profile: F3 makes certificates strictly longer (every read, not just logged ones), and F2 adds a per-slot iteration order over writer's-world memos. Certificate **validation is the inner loop of every marked-cone read** — the exact workload gate G-8 (≤1.5× DIRECT during a live batch) prices — and the F2 drain re-validation runs per deferred drain (G-7). That is the profile this design packs everywhere else. Memos also already have plane `G`'s lifecycle: bump-allocated, bulk-invalidated, zero residue at quiescence. They should be records, not objects:

- **Records** (stride 8): `KEY` (world key, 10.5 encoding) · `EPOCH` (overlayEpoch at eval; 0 reserved as tombstone — never a live epoch) · `NODE` (owning computed, needed by the F2 drain) · `VAL` (index into a `memoVals` side array — the value/box ref is GC-visible and must stay on the heap) · `NEXT_MEMO` (this node's memo chain; the chain head becomes an integer in the existing `memos[id >> 3]` slot, and `MEMO_KEY` at `M[+7]` keeps mirroring the first live entry's key) · `SLOT_NEXT` (writer's-world records only: the F2 per-slot chain, heads in `slotMemoHead: Int32Array(32)`) · `NDEPS` · `CERT` (offset of the certificate run). Certificate pairs `(atomId, seqOrZero)` are bump-allocated in a companion region of the same plane. Keeping the header fixed-stride (rather than inlining variable-length certificates) preserves the record-iteration idioms the debug twin, `verifyArena`, and the schema/codegen pipeline are built on; the certificate scan is contiguous either way, at the cost of one extra load per validation. If that load ever shows in profiles, inline variable-length records are the fallback — measured, not assumed.
- **Validation** is a forward Int32 scan: for each pair, `(M[aid+FLAGS] & LOGGED ? G[M[aid+LOG_TAIL]+SEQ] : 0) === seq`. Monomorphic, no pointer chasing. The heap version spends three dependent loads (entry object → two arrays) before the loop starts.
- **Collection** during evaluation uses a persistent scratch `Int32Array` with saved frame bases (the `propStack` discipline). F3's nested-frame flattening falls out as a base-index rule: an inner evaluation's span simply stays appended under every open frame's base; a child memo *hit* copies the child record's certificate run into the scratch. The record is written once at frame exit.
- **Lifecycle:** re-memoizing a (node, key) appends a fresh record and tombstones the old (`EPOCH = 0`, `memoVals[VAL] = undefined` — the same freed-slot hygiene 16.6 already sweeps for); sweep trims chains opportunistically exactly as 10.5 specifies for lists; the plane, its certificate region, and `memoVals` reset at quiescence (F8's epoch bump makes any surviving reference provably dead). Mid-evaluation allocation follows the watermark-slack rule the other planes obey — allocate from slack, grow only at operation boundaries, where (per F11) yield gaps qualify. One hazard the reset creates and the design must own: node memo heads (`memos[id >> 3]`) are not walked at quiescence, so they can dangle into recycled offsets of the reset plane; the lookup therefore guards with `W[rec + NODE] === nodeId` (mismatch ⇒ node has no memos, zero the head lazily) — the same stale-reference defense `GEN` provides for disposers, and a `verifyArena` check.
- **Accounting:** the 7.6 row becomes "32 B header + 8 B per dependency + one `memoVals` slot", and overlay evaluation stops allocating engine-side objects entirely — which *tightens* 14.3's steady-state allocation story rather than adding to it.

Plane `W` is `tools/schema.ts` material like every other plane: record family and field table, generated docs, debug-twin hydrators, and invariant sweeps (memo chains acyclic and terminating; `SLOT_NEXT` nonzero only on writer-key records; tombstones' `memoVals` slots undefined).

### Adopted: a token column on the broadcast queue (serves F6)

Entries become `(watcherId, token)` stride-2 pairs in the existing flat queue; `propagate` pushes token 0 (urgent), the notify walk pushes its write's token; the drain groups by token and wraps deferred groups in `unstable_runInBatch`. One extra store per enqueue, on paths G-6/G-7 already gate.

### Priced and rejected: new per-node state in plane `M`

An O(1) "which slots' worlds memoized me" mask (F2 dedup) or a packed memo-head field would want a ninth node slot. Node records are full (+0..+7 assigned for every kind), so the real options are:

- **Stride 16:** +32 B on *every* node and link for a cold overlay feature — doubles the 7.6 byte story, halves records per cache line on the hot canonical walks, and re-prices every G-1/G-2 number. Rejected.
- **A cold parallel column** (`Int32Array` indexed `id >> 3`): layout-legal — the measured 1.8× parallel-array penalty was for *hot traversal* fields, and this would be touched only on overlay paths — but currently unnecessary: registration dedup falls out of the memo-chain lookup `overlayEvaluate` performs anyway, and the memo head fits the existing `memos` side slot as an integer. If the overlay ever demands a per-node word, this is the shape to reach for — never stride growth, and never a flags bit (the remaining flag bits are the kernel's headroom; F4 deliberately reuses `OVERLAY_STAMP`/`LOGGED` instead of minting a "marked" bit).
- **Overlay edges threaded into the canonical intrusive lists:** still rejected outright — a per-link branch in `propagate`/`checkDirty` breaks monomorphism and exact pull counts; keeping world bookkeeping out of kernel walks is the reason the F2 drain design exists. If the F2 drain scan ever shows up in profiles, the escalation is codex's (source, world) → consumer **edge plane** with quiescence reset — displacing the memo-chain design on measurement, not on architectural taste; it is bounded by *touched nodes per live world*, which transitions keep small.

### Where every fix lands

| fix | layer | data | hot-path delta | gate that prices it |
| --- | --- | --- | --- | --- |
...
| F2 slot chains + drain re-validation | overlay | plane `W` `SLOT_NEXT` chains + `slotMemoHead(32)` | zero outside drains; per-drain: certificate scans over the slot's chain | G-7 (add registered-node dimension) |
| F3 full certificates + flattening + settlement epoch | overlay | certificate runs in plane `W`; persistent scratch collector | validation covers all deps — one contiguous Int32 scan | G-8 |
...
| F6 token column + `runInBatch` drain groups | kernel queue + bindings | stride-2 broadcast queue | one extra store per enqueue; one `runInBatch` per distinct token per drain | G-7 |
...
| F11 allow plane growth in yield gaps | kernel alloc | none | none (removes a throw) | growth-stress suite |
