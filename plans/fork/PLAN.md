...
- **Target: `e71a6393e66b`** — it is HEAD _and_ has published canary +
...
parked action token _is_ the re-wrap token — the write-set-closure late-write
...
| 15 per-root facts, two roots, spanning transition                     | **partial→new** | T7 is the seed (two `createRoot`s, suspended second root, lock-in + exactly-once) but asserts no per-root commit _event_; **existence proof — first** |
...
1. **Per-root commit facts under multi-root schedules** — tests 15–17 + 25.
   ...
   spanning both, per-root commit _reported_ with generation while the token
...
the `renderRootConcurrent` entry path that _skips_ `prepareFreshStack`
